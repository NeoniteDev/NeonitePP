...
struct NeoniteGUI
{
	...
	{
		NeoniteGUI* console = (NeoniteGUI*)data->UserData;
	...
	int TextEditCallback(ImGuiInputTextCallbackData* data)
	{
		...
		case ImGuiInputTextFlags_CallbackCompletion:
		{
			...
			for (int i = 0; i < Commands.Size; i++) if (Strnicmp(Commands[i], word_start, (int)(word_end - word_start)) == 0) candidates.push_back(Commands[i]);
		...
		case ImGuiInputTextFlags_CallbackHistory:
		{
			...
			{
				if (HistoryPos != -1) if (++HistoryPos >= History.Size) HistoryPos = -1;
			}
