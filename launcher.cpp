...
void launcher()
{
	...
	oss << " -epicapp=Fortnite -epicenv=Prod -epiclocale=en-us -epicportal -noeac -nobe -fltoken=8c4aa8a9b77acdcbd918874b -AUTH_LOGIN="
	<< name
	...
	if (hClient && hClient != INVALID_HANDLE_VALUE)
	{
		...
		if (!addDll.empty())
		{
			...
			{
				CloseHandle(hClient);
				console.AddLog("[x] The additional DLL couldn't be injected, an error was encountered.");
				return;
			}
		...
		{
			console.AddLog("[=] Skipping Platanium.dll injection due to user choice.");
		}
