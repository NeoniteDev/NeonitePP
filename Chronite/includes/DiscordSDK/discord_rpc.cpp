...
static bool RegisterForEvent(const char* evtName)
{
	...
	{
		qmessage->length =
		JsonWriteSubscribeCommand(qmessage->buffer, sizeof(qmessage->buffer),
		                          Nonce++, evtName);
		SendQueue.CommitAdd();
		SignalIOActivity();
		return true;
...
static bool DeregisterForEvent(const char* evtName)
{
	...
	{
		qmessage->length =
		JsonWriteUnsubscribeCommand(qmessage->buffer, sizeof(qmessage->buffer),
		                            Nonce++, evtName);
		SendQueue.CommitAdd();
		SignalIOActivity();
		return true;
...
extern "C" DISCORD_EXPORT void Discord_UpdatePresence(
                                               ...
                                               /* DISCORD_REPLY_ */ int reply)
{
	...
	{
		qmessage->length =
		JsonWriteJoinReply(qmessage->buffer, sizeof(qmessage->buffer), userId,
		                   reply, Nonce++);
		SendQueue.CommitAdd();
		SignalIOActivity();
	}
...
extern "C" DISCORD_EXPORT void Discord_RunCallbacks(void)
{
	...
	while (JoinAskQueue.HavePendingSends())
	{
		auto req = JoinAskQueue.GetNextSendMessage();
		{
			...
			{
				DiscordUser du{
					req->userId, req->username, req->discriminator,
					req->avatar
				};
...
extern "C" DISCORD_EXPORT void Discord_UpdateHandlers(
	DiscordEventHandlers* newHandlers)
{
	if (newHandlers)
	{
