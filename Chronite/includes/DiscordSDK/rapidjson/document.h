	...
	bool RemoveMember(const std::basic_string<Ch>& name) { return RemoveMember(GenericValue(StringRef(name))); }
		...
		*/
		MemberIterator RemoveMember(MemberIterator m)
		{
			...
			if (data_.o.size > 1 && m != last) *m = *last; // Move the last one to this place
			else m->~Member(); // Only one left, just destroy
			--data_.o.size;
			return m;
		}
	...
	*/
		...
		bool Accept(Handler& handler) const
		{
			...
			case kObjectType:
				...
				for (ConstMemberIterator m = MemberBegin(); m != MemberEnd(); ++m)
				{
					...
					if (RAPIDJSON_UNLIKELY(!handler.Key(m->name.GetString(), m->name.GetStringLength(), (m->name.data_.f.flags & kCopyFlag) != 0))) return false;
			...
			default:
				...
				if (IsUint()) return handler.Uint(data_.n.u.u);
				if (IsInt64()) return handler.Int64(data_.n.i64);
	...
	private:
		...
		void SetArrayRaw(GenericValue* values, SizeType count, Allocator& allocator)
		{
			...
			else SetElementsPointer(nullptr);
			data_.a.size = data_.a.capacity = count;
		}
		...
		void SetObjectRaw(Member* members, SizeType count, Allocator& allocator)
		{
			...
			else SetMembersPointer(nullptr);
			data_.o.size = data_.o.capacity = count;
		}
...
*/
	...
	*/
	...
	public:
		...
		{
			if (copy) new(stack_.template Push<ValueType>()) ValueType(str, length, GetAllocator());
...
#endif
		template <typename T>
		RAPIDJSON_DISABLEIF_RETURN((internal::OrExpr<internal::IsPointer<T>, internal::IsGenericValue<T> >), (ValueType&)) AddMember(ValueType& name, T value, AllocatorType& allocator) const
		{
			value_.AddMember(name, value, allocator);
			return *this;
		}
