	...
	public:
		...
		*/
		...
		Writer(StackAllocator* allocator = nullptr, size_t levelDepth = kDefaultLevelDepth) :
			os_(nullptr), level_stack_(allocator, levelDepth * sizeof(Level)), maxDecimalPlaces_(kDefaultMaxDecimalPlaces), hasRoot_(false)
		{
		}
...
#undef Z16
			...
			if (TargetEncoding::supportUnicode) PutReserve(*os_, 2 + length * 6); // "\uxxxx..."
			else PutReserve(*os_, 2 + length * 12); // "\uxxxx\uyyyy..."
		...
		void Prefix(Type type)
		{
			...
			if (RAPIDJSON_LIKELY(level_stack_.GetSize() != 0))
			{
				...
				{
					if (level->inArray) os_->Put(','); // add comma if it is not the first element in array
					else // in object
	...
	inline bool Writer<StringBuffer>::WriteDouble(double d)
	{
		if (internal::Double(d).IsNanOrInf())
		{
			...
			if constexpr (!(kWriteDefaultFlags & kWriteNanAndInfFlag)) return false;
			if (internal::Double(d).IsNan())
			{
				PutReserve(*os_, 3);
				PutUnsafe(*os_, 'N');
