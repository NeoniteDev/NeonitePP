	...
	namespace internal
	{
		...
		*/
		...
		public:
			...
			GenericRegex(const Ch* source, Allocator* allocator = nullptr) :
				ownAllocator_(allocator ? nullptr : RAPIDJSON_NEW(Allocator)()), allocator_(allocator ? allocator : ownAllocator_),
				...
				*atomCountStack.template Push<unsigned>() = 0;
				...
				while (ds.Peek() != 0)
				{
					...
					case '{':
					{
						...
						{
							ds.Take();
							if (ds.Peek() == '}') m = kInfinityQuantifier;
							else if (!ParseUnsigned(ds, &m) || m < n) return;
						}
						else m = n;
			...
			bool EvalQuantifier(Stack<Allocator>& operandStack, unsigned n, unsigned m)
			{
				...
				if (n == 0)
				{
					if (m == 0) // a{0} not support
						return false;
					if (m == kInfinityQuantifier) Eval(operandStack, kZeroOrMore); // a{0,} -> a*
					else
					{
				...
				if (m == kInfinityQuantifier) Eval(operandStack, kOneOrMore); // a{3,} -> a a a+
				else if (m > n)
				{
				...
				for (unsigned i = 0; i < n - 1; i++) Eval(operandStack, kConcatenation); // a{3} -> aaa, a{3,} -> aaa+, a{3.5} -> aaaa?a?
			...
			bool ParseRange(DecodedStream<InputStream, Encoding>& ds, SizeType* range)
			{
				...
				while ((codepoint = ds.Take()) != 0)
				{
					...
					case '\\':
						...
						else if (!CharacterEscape(ds, &codepoint)) return false;
						// fall through to default
		...
		public:
			...
			GenericRegexSearch(const RegexType& regex, Allocator* allocator = nullptr) :
				regex_(regex), allocator_(allocator), ownAllocator_(nullptr),
		...
		private:
			...
			bool AddState(Stack<Allocator>& l, SizeType index)
			{
				...
				if (!(stateSet_[index >> 5] & (1u << (index & 31))))
				{
