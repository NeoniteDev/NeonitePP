...
static void ImGui_ImplDX9_CreateWindow(ImGuiViewport* viewport) {
	...
	HRESULT hr = g_pd3dDevice->CreateAdditionalSwapChain(
		&data->d3dpp, &data->SwapChain);
	IM_UNUSED(hr);
	IM_ASSERT(hr == D3D_OK);
...
static void ImGui_ImplDX9_DestroyWindow(ImGuiViewport* viewport) {
	...
	if (ImGuiViewportDataDx9* data = (ImGuiViewportDataDx9*)viewport->
		RendererUserData) {
		if (data->SwapChain)
...
static void ImGui_ImplDX9_SetWindowSize(ImGuiViewport* viewport, ImVec2 size) {
	ImGuiViewportDataDx9* data = (ImGuiViewportDataDx9*)viewport->
		RendererUserData;
	if (data->SwapChain) {
		data->SwapChain->Release();
		data->SwapChain = NULL;
...
static void ImGui_ImplDX9_SwapBuffers(ImGuiViewport* viewport, void*) {
	ImGuiViewportDataDx9* data = (ImGuiViewportDataDx9*)viewport->
		RendererUserData;
	HRESULT hr = data->SwapChain->Present(NULL, NULL, data->d3dpp.hDeviceWindow,
		NULL, NULL);
	IM_ASSERT(hr == D3D_OK);
}
