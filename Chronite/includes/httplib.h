  ...
  return std::string();
		                                     ...
		                                     const std::string& intf, Error& error)
		{
          ...
          if (!bind_ip_address(sock, ip.c_str())) {
			...
			{
				if (error == Success) { error = Connection; }
			}
...
inline std::pair<std::string, std::string> make_digest_authentication_header(
  ...
  return std::make_pair(key, field);
	...
	inline void Response::set_content_provider(size_t in_length, const char* content_type,
  ...
  signal(SIGPIPE, SIG_IGN);
	...
	inline bool Server::write_content_with_provider(Stream& strm, const Request& req,
	                                                Response& res, const std::string& boundary,
	                                                const std::string& content_type)
	{
  ...
  assert(socket.ssl == nullptr);
	...
	inline bool ClientImpl::send(const Request& req, Response& res, Error& error)
	{
      ...
      if (is_ssl()) {
		...
		if (!ret)
		{
			if (error == Success) { error = Unknown; }
		}
  ...
  if (compress_ && !content_provider_without_length) {
	                               ...
	                               const std::string& boundary)
	{
		for (size_t i = 0; i < boundary.size(); i++)
		{
			...
			{
				return Result{nullptr, UnsupportedMultipartBoundaryChars};
			}
		}
