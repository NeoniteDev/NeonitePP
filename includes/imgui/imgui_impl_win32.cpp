...
bool ImGui_ImplWin32_Init(void* hwnd) {
	...
	main_viewport->PlatformHandle =
		main_viewport->PlatformHandleRaw = (void*)g_hWnd;
	...
	if (HWND focused_hwnd = ::GetForegroundWindow()) {
		...
		if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable) {
			...
			if (ImGui::FindViewportByPlatformHandle((void*)focused_hwnd) != NULL)
				io.MousePos = ImVec2((float)mouse_screen_pos.x,
			...
			if (focused_hwnd == g_hWnd) {
				...
				io.MousePos = ImVec2((float)mouse_client_pos.x,
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(
	...
	case WM_MOUSEWHEEL:
		...
		return 0;
	case WM_MOUSEHWHEEL:
		io.MouseWheelH += (float)GET_WHEEL_DELTA_WPARAM(wParam) / (float)
			WHEEL_DELTA;
		return 0;
	case WM_KEYDOWN:
	case WM_SYSKEYDOWN:
...
float ImGui_ImplWin32_GetDpiScaleForMonitor(void* monitor) {
	...
	if (::IsWindows8Point1OrGreater()) {
		static HINSTANCE shcore_dll = ::LoadLibraryA("shcore.dll");
		// Reference counted per-process
		if (PFN_GetDpiForMonitor GetDpiForMonitorFn = (PFN_GetDpiForMonitor)
...
static void ImGui_ImplWin32_CreateWindow(ImGuiViewport* viewport) {
	...
	ImGui_ImplWin32_GetWin32StyleFromViewportFlags(
		viewport->Flags, &data->DwStyle, &data->DwExStyle);
	HWND parent_window = NULL;
	if (viewport->ParentViewportId != 0)
		if (ImGuiViewport* parent_viewport = ImGui::FindViewportByID(
...
static void ImGui_ImplWin32_DestroyWindow(ImGuiViewport* viewport) {
	if (ImGuiViewportDataWin32* data = (ImGuiViewportDataWin32*)viewport->
		PlatformUserData) {
...
static void ImGui_ImplWin32_ShowWindow(ImGuiViewport* viewport) {
	ImGuiViewportDataWin32* data = (ImGuiViewportDataWin32*)viewport->
		PlatformUserData;
...
static void ImGui_ImplWin32_UpdateWindow(ImGuiViewport* viewport) {
	...
	DWORD new_ex_style;
	ImGui_ImplWin32_GetWin32StyleFromViewportFlags(
		viewport->Flags, &new_style, &new_ex_style);
	...
	if (data->DwStyle != new_style || data->DwExStyle != new_ex_style) {
		...
		::AdjustWindowRectEx(&rect, data->DwStyle, FALSE, data->DwExStyle);
		// Client to Screen
		::SetWindowPos(data->Hwnd, NULL, rect.left, rect.top,
...
static void
ImGui_ImplWin32_SetWindowSize(ImGuiViewport* viewport, ImVec2 size) {
	ImGuiViewportDataWin32* data = (ImGuiViewportDataWin32*)viewport->
		PlatformUserData;
	IM_ASSERT(data->Hwnd != 0);
	...
	::AdjustWindowRectEx(&rect, data->DwStyle, FALSE, data->DwExStyle);
	// Client to Screen
	::SetWindowPos(data->Hwnd, NULL, 0, 0, rect.right - rect.left,
...
static BOOL CALLBACK ImGui_ImplWin32_UpdateMonitors_EnumFunc(
	HMONITOR monitor, HDC, LPRECT, LPARAM) {
	MONITORINFO info = { 0 };
	info.cbSize = sizeof(MONITORINFO);
	if (!::GetMonitorInfo(monitor, &info))
		return TRUE;
	ImGuiPlatformMonitor imgui_monitor;
	imgui_monitor.MainPos = ImVec2((float)info.rcMonitor.left,
	...
	imgui_monitor.WorkPos = ImVec2((float)info.rcWork.left,
...
static void ImGui_ImplWin32_UpdateMonitors() {
	...
	::EnumDisplayMonitors(NULL, NULL, ImGui_ImplWin32_UpdateMonitors_EnumFunc,
		NULL);
