...
static inline ImU32 ImHash(const void *data, int size, ImU32 seed = 0) {
  return size
           ? ImHashData(data, (size_t)size, seed)
...
struct IMGUI_API ImRect {
  ...
  bool Contains(const ImRect &r) const {
    return r.Min.x >= Min.x && r.Min.y >= Min.y && r.Max.x <= Max.x && r.Max.y
           <= Max.y;
  }

  bool Overlaps(const ImRect &r) const {
    return r.Min.y < Max.y && r.Max.y > Min.y && r.Min.x < Max.x && r.Max.x >
           Min.x;
  }
...
struct ImGuiViewportP : public ImGuiViewport {
  ...
  ImGuiViewportP() {
    Idx = -1;
    LastFrameActive = LastFrameDrawLists[0] =
                      LastFrameDrawLists[1] = LastFrontMostStampCount = -1;
    LastNameHash = 0;
    ...
    LastPlatformPos = LastPlatformSize = LastRendererSize = ImVec2(
                                             FLT_MAX, FLT_MAX);
  }
...
enum ImGuiDockNodeFlagsPrivate_ {
  ...
  ImGuiDockNodeFlags_NoResize | ImGuiDockNodeFlags_DockSpace |
  ImGuiDockNodeFlags_CentralNode | ImGuiDockNodeFlags_NoTabBar |
  ImGuiDockNodeFlags_HiddenTabBar | ImGuiDockNodeFlags_NoWindowMenuButton |
  ImGuiDockNodeFlags_NoCloseButton
};
...
struct ImGuiContext {
  ...
  ImGuiStyle Style;
  ImGuiConfigFlags ConfigFlagsForFrame;
  ...
  ImGuiContext(ImFontAtlas *shared_font_atlas) {
    ...
    NavWindow = NULL;
    NavId = NavActivateId = NavActivateDownId =
                            NavActivatePressedId = NavInputId = 0;
    NavJustTabbedId = NavJustMovedToId =
                      NavJustMovedToMultiSelectScopeId = NavNextActivateId = 0;
...
struct IMGUI_API ImGuiWindowTempData {
  ...
  int TreeDepth;
  ImU32 TreeStoreMayJumpToParentOnPop;
  ...
  ImGuiWindowTempData() {
    CursorPos = CursorPosPrevLine =
                CursorStartPos = CursorMaxPos = ImVec2(0.0f, 0.0f);
...
struct IMGUI_API ImGuiWindow {
  ...
  ImGuiID DockId;
  // Backup of last valid DockNode->Id, so single value remember their dock node id
  ...
  // Is the window visible this frame? =~ is the corresponding tab selected?
...
public:
  ...
  float CalcFontSize() const {
    return GImGui->FontBaseSize * FontWindowScale * FontDpiScale;
  }
  ...
  float MenuBarHeight() const {
    return (Flags & ImGuiWindowFlags_MenuBar)
             ? DC.MenuBarOffset.y + CalcFontSize() + GImGui->Style.FramePadding.
               y * 2.0f
             : 0.0f;
  }
...
// Warning: DO NOT HOLD ON ImGuiDockNode* pointer, will be invalided by any split/merge/remove operation.
...
IMGUI_API ImGuiID
DockBuilderAddNode(ImGuiID node_id, ImGuiDockNodeFlags flags = 0);
// Use (flags == ImGuiDockNodeFlags_DockSpace) to create a dockspace, otherwise it'll create a floating node.
IMGUI_API void DockBuilderRemoveNode(ImGuiID node_id);
// Remove node and all its child, undock all windows
IMGUI_API void DockBuilderRemoveNodeDockedWindows(
    ImGuiID node_id, bool clear_persistent_docking_references = true);
...
IMGUI_API void
BeginColumns(const char *str_id, int count, ImGuiColumnsFlags flags = 0);
...
FindRenderedTextEnd(const char *text, const char *text_end = NULL);
...
IMGUI_API void RenderRectFilledWithHole(ImDrawList *draw_list, ImRect outer,
                                        ...
                                        float rounding);
IMGUI_API void RenderPixelEllipsis(ImDrawList *draw_list, ImVec2 pos, ImU32 col,
                                   int count);
...
DragBehaviorT(ImGuiDataType data_type, T *v, float v_speed, T v_min, T v_max,
              const char *format, float power, ImGuiDragFlags flags);
template <typename T, typename SIGNED_T, typename FLOAT_T> IMGUI_API bool
SliderBehaviorT(const ImRect &bb, ImGuiID id, ImGuiDataType data_type, T *v,
                T v_min, T v_max, const char *format, float power,
                ImGuiSliderFlags flags, ImRect *out_grab_bb);
template <typename T, typename FLOAT_T> IMGUI_API float
SliderCalcRatioFromValueT(ImGuiDataType data_type, T v, T v_min, T v_max,
                          float power, float linear_zero_pos);
template <typename T, typename SIGNED_T> IMGUI_API T RoundScalarWithFormatT(
