...
static void SetWindowHitTestHole(ImGuiWindow *window, const ImVec2 &pos,
                                 const ImVec2 &size);
...
void *ImFileLoadToMemory(const char *filename, const char *file_open_mode,
  ...
  if (fseek(f, 0, SEEK_END) || (file_size_signed = ftell(f)) == -1 || fseek(
          f, 0, SEEK_SET)) {
    fclose(f);
    return NULL;
  }
...
int ImGuiStorage::GetInt(ImGuiID key, int default_val) const {
  ImGuiStorage::Pair *it = LowerBound(
...
void ImGui::RenderTextEllipsis(ImDrawList *draw_list, const ImVec2 &pos_min,
  ...
  if (text_size.x > pos_max.x - pos_min.x) {
    ...
    float text_size_clipped_x = font->CalcTextSizeA(
        font_size, text_width, 0.0f, text, text_end_full, &text_end_ellipsis).x;
    ...
    while (text_end_ellipsis > text && ImCharIsBlankA(text_end_ellipsis[-1])) {
      ...
      text_size_clipped_x -= font->CalcTextSizeA(
          font_size, FLT_MAX, 0.0f, text_end_ellipsis, text_end_ellipsis + 1).x;
...
void ImGui::SetActiveID(ImGuiID id, ImGuiWindow *window) {
  ...
  if (id) {
    ...
    g.ActiveIdSource = (g.NavActivateId == id || g.NavInputId == id || g.
                        NavJustTabbedId == id || g.NavJustMovedToId == id)
...
static inline bool IsWindowContentHoverable(ImGuiWindow *window,
  ...
  if (g.NavWindow)
    if (ImGuiWindow *focused_root_window = g.NavWindow->RootWindow)
      if (focused_root_window->WasActive && focused_root_window != window->
          RootWindow) {
  ...
  if (window->Viewport != g.MouseViewport)
    if (g.MovingWindow == NULL || window->RootWindow != g.MovingWindow->
        RootWindow)
      return false;
...
void ImGui::ItemSize(const ImVec2 &size, float text_offset_y) {
  ...
  const float text_base_offset = ImMax(window->DC.CurrLineTextBaseOffset,
                                       text_offset_y);
  //if (g.IO.KeyAlt) window->DrawList->AddRect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size.x, line_height), IM_COL32(255,0,0,200)); // [DEBUG]
  window->DC.CursorPosPrevLine.x = window->DC.CursorPos.x + size.x;
  window->DC.CursorPosPrevLine.y = window->DC.CursorPos.y;
  window->DC.CursorPos.x = (float)(int)(
    window->Pos.x + window->DC.Indent.x + window->DC.ColumnsOffset.x);
  window->DC.CursorPos.y = (float)(int)(
    window->DC.CursorPos.y + line_height + g.Style.ItemSpacing.y);
  ...
  window->DC.CursorMaxPos.y = ImMax(window->DC.CursorMaxPos.y,
                                    window->DC.CursorPos.y - g.Style.ItemSpacing
                                    .y);
...
bool ImGui::IsItemHovered(ImGuiHoveredFlags flags) {
  ...
  if (!(flags & ImGuiHoveredFlags_AllowWhenBlockedByActiveItem))
    if (g.ActiveId != 0 && g.ActiveId != window->DC.LastItemId && !g.
        ActiveIdAllowOverlap && g.ActiveId != window->MoveId)
      return false;
  ...
  if ((window->DC.LastItemId == window->ID || window->DC.LastItemId == window->
       MoveId) && window->WriteAccessed)
    return false;
  return true;
}
...
bool ImGui::ItemHoverable(const ImRect &bb, ImGuiID id) {
  ...
  if (!IsMouseHoveringRect(bb.Min, bb.Max))
    return false;
  if (g.NavDisableMouseHover || !IsWindowContentHoverable(
...
bool ImGui::FocusableItemRegister(ImGuiWindow *window, ImGuiID id) {
  ...
  // Increment counters
  const bool is_tab_stop = (window->DC.ItemFlags & (
  ...
  if (g.ActiveId == id && g.FocusTabPressed && !(
        g.ActiveIdBlockNavInputFlags & (1 << ImGuiNavInput_KeyTab_)) && g.
  ...
  if (g.FocusRequestCurrWindow == window) {
    if (window->DC.FocusCounterAll == g.FocusRequestCurrCounterAll)
      return true;
    if (is_tab_stop && window->DC.FocusCounterTab == g.
...
static ImDrawList *GetViewportDrawList(ImGuiViewportP *viewport,
  ...
  ImGuiContext &g = *GImGui;
  IM_ASSERT(
      drawlist_no >= 0 && drawlist_no < IM_ARRAYSIZE(viewport->DrawLists));
...
void ImGui::StartMouseMovingWindow(ImGuiWindow *window) {
  ...
  if (ImGuiDockNode *node = window->DockNodeAsHost)
    if (node->VisibleWindow && (node->VisibleWindow->Flags &
  ...
  if (g.IO.MouseClicked[0]) {
    if (g.HoveredRootWindow != NULL) {
      ...
      if (g.IO.ConfigWindowsMoveFromTitleBarOnly && (
            !(g.HoveredRootWindow->Flags & ImGuiWindowFlags_NoTitleBar) || g.
            HoveredWindow->RootWindowDockStop->DockIsActive))
...
static void ImGui::UpdateMouseInputs() {
  ...
  for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++) {
    g.IO.MouseClicked[i] = g.IO.MouseDown[i] && g.IO.MouseDownDuration[i] <
                           0.0f;
    g.IO.MouseReleased[i] = !g.IO.MouseDown[i] && g.IO.MouseDownDuration[i] >=
                            0.0f;
    g.IO.MouseDownDurationPrev[i] = g.IO.MouseDownDuration[i];
    g.IO.MouseDownDuration[i] = g.IO.MouseDown[i]
                                  ? (g.IO.MouseDownDuration[i] < 0.0f
                                       ? 0.0f
                                       : g.IO.MouseDownDuration[i] + g.IO.
                                         DeltaTime)
                                  : -1.0f;
    ...
    if (g.IO.MouseClicked[i]) {
      if ((float)(g.Time - g.IO.MouseClickedTime[i]) < g.IO.MouseDoubleClickTime
        ...
        ImVec2 delta_from_click_pos = IsMousePosValid(&g.IO.MousePos)
                                        ? (g.IO.MousePos - g.IO.MouseClickedPos[
                                             i])
                                        : ImVec2(0.0f, 0.0f);
        if (ImLengthSqr(delta_from_click_pos) < g.IO.MouseDoubleClickMaxDist * g
...
void ImGui::UpdateMouseWheel() {
  ...
  if (g.IO.MouseWheel != 0.0f && g.IO.KeyCtrl && g.IO.FontAllowUserScaling && !g
      .HoveredWindow->Collapsed) {
...
void ImGui::UpdateHoveredWindowAndCaptureFlags() {
  ...
  FindHoveredWindow();
  IM_ASSERT(
      g.HoveredWindow == NULL || g.HoveredWindow == g.MovingWindow || g.
      HoveredWindow->Viewport == g.MouseViewport);

  // Modal windows prevents cursor from hovering behind them.
  ImGuiWindow *modal_window = GetTopMostPopupModal();
  if (modal_window)
    if (g.HoveredRootWindow && !IsWindowChildOf(g.HoveredRootWindow,
                                                modal_window))
  ...
  for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++) {
    if (g.IO.MouseClicked[i])
      g.IO.MouseDownOwned[i] = (g.HoveredWindow != NULL) || (!g.OpenPopupStack.
                                 empty());
    mouse_any_down |= g.IO.MouseDown[i];
    if (g.IO.MouseDown[i])
      if (mouse_earliest_button_down == -1 || g.IO.MouseClickedTime[i] < g.IO.
          MouseClickedTime[mouse_earliest_button_down])
        mouse_earliest_button_down = i;
  }
  const bool mouse_avail_to_imgui =
      (mouse_earliest_button_down == -1) || g.IO.MouseDownOwned[
        mouse_earliest_button_down];

  // If mouse was first clicked outside of ImGui bounds we also cancel out hovering.
  // FIXME: For patterns of drag and drop across OS windows, we may need to rework/remove this test (first committed 311c0ca9 on 2015/02)
  const bool mouse_dragging_extern_payload =
      g.DragDropActive && (g.DragDropSourceFlags &
...
void ImGui::NewFrame() {
  ...
  IM_ASSERT(
      g.Style.WindowMenuButtonPosition == ImGuiDir_Left || g.Style.
  ...
  if (g.IO.ConfigFlags & ImGuiConfigFlags_ViewportsEnable) {
    if ((g.IO.BackendFlags & ImGuiBackendFlags_PlatformHasViewports) && (
      ...
      IM_ASSERT(
      (g.Viewports[0]->PlatformUserData != NULL || g.Viewports[0]->
        PlatformHandle != NULL) && "Platform init didn't setup main viewport.");
      ...
      // Call ImGui::Render() then pass ImGui::GetDrawData() yourself to your render function!
#endif
    } else {
    ...
    for (int monitor_n = 0; monitor_n < g.PlatformIO.Monitors.Size; monitor_n++
      ...
      IM_ASSERT(
          ...
          "Monitor bounds not setup properly.");
      IM_ASSERT(
  ...
  g.DrawListSharedData.ClipRectFullscreen = ImVec4(
      virtual_space.Min.x, virtual_space.Min.y, virtual_space.Max.x,
      virtual_space.Max.y);
  ...
  g.FramerateSecPerFrameAccum += g.IO.DeltaTime - g.FramerateSecPerFrame[g.
    FramerateSecPerFrameIdx];
  g.FramerateSecPerFrame[g.FramerateSecPerFrameIdx] = g.IO.DeltaTime;
  g.FramerateSecPerFrameIdx = (g.FramerateSecPerFrameIdx + 1) % IM_ARRAYSIZE(
                                  g.FramerateSecPerFrame);
  g.IO.Framerate = (g.FramerateSecPerFrameAccum > 0.0f)
                     ? (1.0f / (g.FramerateSecPerFrameAccum / (float)
  ...
  if (g.FocusRequestNextWindow != NULL) {
    ...
    if (g.FocusRequestNextCounterAll != INT_MAX && window->DC.FocusCounterAll !=
        -1)
      g.FocusRequestCurrCounterAll = ImModPositive(
          g.FocusRequestNextCounterAll, window->DC.FocusCounterAll + 1);
    if (g.FocusRequestNextCounterTab != INT_MAX && window->DC.FocusCounterTab !=
        -1)
...
static void AddDrawListToDrawData(ImVector<ImDrawList *> *out_list,
  ...
  IM_ASSERT(
      draw_list->VtxBuffer.Size == 0 || draw_list->_VtxWritePtr == draw_list->
      VtxBuffer.Data + draw_list->VtxBuffer.Size);
  IM_ASSERT(
      draw_list->IdxBuffer.Size == 0 || draw_list->_IdxWritePtr == draw_list->
      IdxBuffer.Data + draw_list->IdxBuffer.Size);
...
static void AddWindowToDrawData(ImGuiWindow *window, int layer) {
  ...
  AddDrawListToDrawData(&window->Viewport->DrawDataBuilder.Layers[layer],
                        window->DrawList);
...
static void ImGui::EndFrameDrawDimmedBackgrounds() {
  ...
  if (dim_bg_for_modal || dim_bg_for_window_list)
    for (int viewport_n = 0; viewport_n < g.Viewports.Size; viewport_n++) {
      ...
      if (g.NavWindowingList && viewport == g.NavWindowingList->Viewport)
        continue;
      if (g.NavWindowingTargetAnim && viewport == g.NavWindowingTargetAnim->
                                                    Viewport)
        continue;
  ...
  if (dim_bg_for_window_list) {
    ...
    ImDrawList *draw_list = FindFrontMostVisibleChildWindow(window->RootWindow)
        ->DrawList;
    ...
    if (window->RootWindowDockStop->DockIsActive)
      if (window->RootWindow != window->RootWindowDockStop)
        RenderRectFilledWithHole(draw_list, window->RootWindow->Rect(),
    ...
    if (bb.Contains(window->Viewport->GetRect()))
    ...
    draw_list->AddRect(bb.Min, bb.Max,
                       GetColorU32(ImGuiCol_NavWindowingHighlight,
                                   g.NavWindowingHighlightAlpha), rounding, ~0,
                       3.0f);
...
void ImGui::EndFrame() {
  ...
  if (g.PlatformIO.Platform_SetImeInputPos && (
        g.PlatformImeLastPos.x == FLT_MAX || ImLengthSqr(
            g.PlatformImePos - g.PlatformImeLastPos) > 0.0001f))
    if (g.PlatformImePosViewport && g.PlatformImePosViewport->
...
void ImGui::Render() {
  ...
  for (int n = 0; n != g.Windows.Size; n++) {
    ...
    if (IsWindowActiveAndVisible(window) && (
          window->Flags & ImGuiWindowFlags_ChildWindow) == 0 && window !=
  ...
  for (int n = 0; n < IM_ARRAYSIZE(windows_to_render_top_most); n++)
    if (windows_to_render_top_most[n] &&
      ...
      // NavWindowingTarget is always temporarily displayed as the tp-most window
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  if (g.Viewports[0]->DrawData->CmdListsCount > 0 && g.IO.RenderDrawListsFn !=
      NULL)
...
static void FindHoveredWindow() {
  ...
  ImGuiViewportP *moving_window_viewport = g.MovingWindow
  ...
  for (int i = g.Windows.Size - 1; i >= 0; i--) {
    ...
    if (window->HitTestHoleSize.x != 0) {
      ...
      ImRect hole_bb((float)(window->HitTestHoleOffset.x),
                     ...
                     (float)(window->HitTestHoleOffset.x + window->
                             HitTestHoleSize.x),
                     (float)(window->HitTestHoleOffset.y + window->
                             HitTestHoleSize.y));
...
bool ImGui::IsKeyDown(int user_key_index) {
  if (user_key_index < 0)
    return false;
  IM_ASSERT(
      user_key_index >= 0 && user_key_index < IM_ARRAYSIZE(GImGui->IO.KeysDown
      ));
...
int ImGui::CalcTypematicPressedRepeatAmount(float t, float t_prev,
  ...
  if (t <= repeat_delay || repeat_rate <= 0.0f)
    return 0;
  const int count = (int)((t - repeat_delay) / repeat_rate) - (int)(
                      (t_prev - repeat_delay) / repeat_rate);
...
bool ImGui::IsKeyReleased(int user_key_index) {
  ...
  IM_ASSERT(
      user_key_index >= 0 && user_key_index < IM_ARRAYSIZE(g.IO.KeysDown));
  return g.IO.KeysDownDurationPrev[user_key_index] >= 0.0f && !g.IO.KeysDown[
           user_key_index];
}
...
bool ImGui::IsMouseDragging(int button, float lock_threshold) {
  ...
  return g.IO.MouseDragMaxDistanceSqr[button] >= lock_threshold *
         lock_threshold;
}
...
ImVec2 ImGui::GetMouseDragDelta(int button, float lock_threshold) {
  ...
  if (g.IO.MouseDown[button] || g.IO.MouseReleased[button])
    if (g.IO.MouseDragMaxDistanceSqr[button] >= lock_threshold * lock_threshold)
      if (IsMousePosValid(&g.IO.MousePos) && IsMousePosValid(
...
bool ImGui::IsItemActivated() {
  ...
  if (g.ActiveId) {
    ImGuiWindow *window = g.CurrentWindow;
    if (g.ActiveId == window->DC.LastItemId && g.ActiveIdPreviousFrame != window
        ->DC.LastItemId)
      return true;
  }
  return false;
...
bool ImGui::IsItemDeactivated() {
  ...
  if (window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_HasDeactivated)
    return (window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_Deactivated)
           != 0;
  return (g.ActiveIdPreviousFrame == window->DC.LastItemId && g.
          ActiveIdPreviousFrame != 0 && g.ActiveId != window->DC.LastItemId);
}
...
static bool ImGui::BeginChildEx(const char *name, ImGuiID id,
  ...
  if (g.NavActivateId == id && !(flags & ImGuiWindowFlags_NavFlattened) && (
        child_window->DC.NavLayerActiveMask != 0 || child_window->DC.
        NavHasScroll)) {
...
static void CheckStacksSize(ImGuiWindow *window, bool write) {
  ...
  short *p_backup = &window->DC.StackSizesBackup[0];
  {
    ...
    if (write)
      *p_backup = (short)current;
      ...
      IM_ASSERT(
        *p_backup == current && "PushID/PopID or TreeNode/TreePop Mismatch!");
    p_backup++;
  ...
  // For color, style and font stacks there is an incentive to use Push/Begin/Pop/.../End patterns, so we relax our checks a little to allow them.
  {
    ...
    if (write)
      *p_backup = (short)current;
      ...
      IM_ASSERT(
        *p_backup >= current && "PushStyleColor/PopStyleColor Mismatch!");
    p_backup++;
  ...
  IM_ASSERT(
      p_backup == window->DC.StackSizesBackup + IM_ARRAYSIZE(window->DC.
        StackSizesBackup));
}
...
static ImVec2 CalcSizeAfterConstraint(ImGuiWindow *window, ImVec2 new_size) {
  ...
  if (!(window->Flags & (ImGuiWindowFlags_ChildWindow |
    ...
    new_size.y = ImMax(new_size.y,
                       window_for_height->TitleBarHeight() + window_for_height->
                       MenuBarHeight() + ImMax(
...
static ImVec2 CalcContentSize(ImGuiWindow *window) {
  ...
  if (window->Hidden && window->HiddenFramesCannotSkipItems == 0 && window->
  ...
  sz.x = (float)(int)((window->ContentSizeExplicit.x != 0.0f)
                        ...
                        : window->DC.CursorMaxPos.x - window->DC.CursorStartPos.
                          x);
...
static ImVec2
CalcSizeAutoFit(ImGuiWindow *window, const ImVec2 &size_contents) {
  ImGuiContext &g = *GImGui;
  ImGuiStyle &style = g.Style;
  ImVec2 size_decorations = ImVec2(
      0.0f, window->TitleBarHeight() + window->MenuBarHeight());
...
ImVec2 ImGui::CalcWindowExpectedSize(ImGuiWindow *window) {
  ...
  return CalcSizeAfterConstraint(
      window, CalcSizeAutoFit(window, size_contents));
}
...
static ImVec2 CalcNextScrollFromScrollTargetAndClamp(
  ...
  if (window->ScrollTarget.x < FLT_MAX) {
    ...
    if (snap_on_edges && cr_x <= 0.0f && target_x <= window->WindowPadding.x)
      target_x = 0.0f;
    else if (snap_on_edges && cr_x >= 1.0f && target_x >= window->ContentSize.x
             + window->WindowPadding.x + GImGui->Style.ItemSpacing.x)
  ...
  if (window->ScrollTarget.y < FLT_MAX) {
    ...
    if (snap_on_edges && cr_y <= 0.0f && target_y <= window->WindowPadding.y)
      target_y = 0.0f;
    if (snap_on_edges && cr_y >= 1.0f && target_y >= window->ContentSize.y +
        window->WindowPadding.y + g.Style.ItemSpacing.y)
...
static bool ImGui::UpdateManualResize(ImGuiWindow *window,
  ...
  for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++
    ...
    const ImVec2 corner = ImLerp(window->Pos, window->Pos + window->Size,
                                 grip.CornerPosN);
    ...
    bool hovered, held;
    ButtonBehavior(resize_rect, window->GetID((void *)(intptr_t)resize_grip_n),
                   &hovered, &held,
  ...
  if (g.NavWindowingTarget && g.NavWindowingTarget->RootWindow == window) {
    ...
    if (nav_resize_delta.x != 0.0f || nav_resize_delta.y != 0.0f) {
      ...
      nav_resize_delta *= ImFloor(NAV_RESIZE_SPEED * g.IO.DeltaTime * ImMin(
...
static void ImGui::RenderWindowOuterBorders(ImGuiWindow *window) {
  ...
  if (g.Style.FrameBorderSize > 0 && !(
    ...
    window->DrawList->AddLine(ImVec2(window->Pos.x + border_size, y),
                              ImVec2(
                                  window->Pos.x + window->Size.x - border_size,
                                  y), GetColorU32(ImGuiCol_Border),
  ...
  if (window->Collapsed) {
    ...
    if (flags & ImGuiWindowFlags_MenuBar) {
      ...
      window->DrawList->AddRectFilled(
          ...
          ImDrawCornerFlags_Top);
      if (style.FrameBorderSize > 0.0f && menu_bar_rect.Max.y < window->Pos.y +
          window->Size.y)
    ...
    if (window->DockNode && window->DockNode->IsHiddenTabBar() && !window->
        DockNode->IsNoTabBar()) {
      ...
      ImU32 col = GetColorU32(
          ((held && hovered) || (window->DockNode->IsFocused && !hovered))
    ...
    if (handle_borders_and_resize_grips && !(flags & ImGuiWindowFlags_NoResize)
      ...
      for (int resize_grip_n = 0; resize_grip_n < resize_grip_count;
        ...
        window->DrawList->PathArcToFast(
            ImVec2(
                corner.x + grip.InnerDir.x * (
                  window_rounding + window_border_size),
                corner.y + grip.InnerDir.y * (
                  window_rounding + window_border_size)), window_rounding,
...
void ImGui::RenderWindowTitleBarContents(ImGuiWindow *window,
  ...
  if (has_close_button) {
    ...
    close_button_pos = ImVec2(
        title_bar_rect.Max.x - pad_r - style.FramePadding.x,
  ...
  if (has_collapse_button && style.WindowMenuButtonPosition == ImGuiDir_Left) {
    collapse_button_pos = ImVec2(
        title_bar_rect.Min.x + pad_l - style.FramePadding.x,
        title_bar_rect.Min.y);
    pad_l += button_sz;
  }
  ...
  const float marker_size_x = (flags & ImGuiWindowFlags_UnsavedDocument)
                                ? CalcTextSize(UNSAVED_DOCUMENT_MARKER, NULL,
                                               false).x
                                : 0.0f;
  const ImVec2 text_size = CalcTextSize(name, NULL, true) + ImVec2(
                               marker_size_x, 0.0f);
...
void ImGui::UpdateWindowParentAndRootLinks(ImGuiWindow *window,
  ...
  window->RootWindow = window->RootWindowDockStop =
  ...
  if (parent_window && !(flags & ImGuiWindowFlags_Modal) && (
    ...
    window->RootWindowForTitleBarHighlight = parent_window->
...
bool ImGui::Begin(const char *name, bool *p_open, ImGuiWindowFlags flags) {
  ...
  if (first_begin_of_the_frame) {
    ...
    bool new_auto_dock_node = !has_dock_node && g.IO.
                                                  ConfigDockingTabBarOnSingleWindows
                              && !(flags & (
  ...
  if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasPos) {
    window_pos_set_by_api = (window->SetWindowPosAllowFlags & g.NextWindowData.
                             PosCond) != 0;
    if (window_pos_set_by_api && ImLengthSqr(g.NextWindowData.PosPivotVal) >
        0.00001f) {
  ...
  if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSize) {
    window_size_x_set_by_api = (window->SetWindowSizeAllowFlags & g.
                                NextWindowData.SizeCond) != 0 && (
                                 g.NextWindowData.SizeVal.x > 0.0f);
    window_size_y_set_by_api = (window->SetWindowSizeAllowFlags & g.
                                NextWindowData.SizeCond) != 0 && (
  ...
  if (first_begin_of_the_frame) {
    ...
    if ((window->Viewport && window->Viewport->Window == window) || (window->
          DockIsActive))
      ...
      window_title_visible_elsewhere = true;
    if (window_title_visible_elsewhere && !window_just_created && strcmp(
            name, window->Name) != 0) {
    ...
    if (window_just_created && (!window_size_x_set_by_api || !
    ...
    if (!window->DockIsActive && (flags & ImGuiWindowFlags_ChildWindow) && !(
      ...
      window->WindowPadding = ImVec2(
          0.0f, (flags & ImGuiWindowFlags_MenuBar)
    ...
    if (!(flags & ImGuiWindowFlags_NoTitleBar) && !(
      ...
      if (g.HoveredWindow == window && g.HoveredId == 0 && g.
          HoveredIdPreviousFrame == 0 && IsMouseHoveringRect(
              title_bar_rect.Min, title_bar_rect.Max) && g.IO.MouseDoubleClicked
          [0])
        window->WantCollapseToggle = true;
    ...
    if (flags & ImGuiWindowFlags_ChildWindow) {
      ...
      window->BeginOrderWithinParent = (short)parent_window->DC.ChildWindows.
          Size;
    ...
    const bool window_pos_with_pivot = (
      window->SetWindowPosVal.x != FLT_MAX && window->
      HiddenFramesCannotSkipItems == 0);
    if (window_pos_with_pivot)
      SetWindowPos(window, ImMax(style.DisplaySafeAreaPadding,
                                 window->SetWindowPosVal - window->SizeFull *
                                 window->SetWindowPosPivot), 0);
    ...
    if (window->ViewportAllowPlatformMonitorExtend >= 0 && !window->
        ViewportOwned && !(window->Viewport->Flags &
    ...
    if (window->ViewportOwned) {
      ...
      if (window->WindowClass.ParentViewportId)
        window->Viewport->ParentViewportId = window->WindowClass.
            ParentViewportId;
      else if ((flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_Tooltip)) &&
               parent_window_in_stack)
        window->Viewport->ParentViewportId = parent_window_in_stack->Viewport->
            ID;
      else
      ...
      if (window->WindowClass.ViewportFlagsOverrideMask)
        viewport_flags = (viewport_flags & ~window->WindowClass.
                                                    ViewportFlagsOverrideMask) |
                         (window->WindowClass.ViewportFlagsOverrideValue &
                          window->WindowClass.ViewportFlagsOverrideMask);
    ...
    if (!window_pos_set_by_api && !(flags & ImGuiWindowFlags_ChildWindow) &&
      ...
      ImVec2 clamp_padding = ImMax(style.DisplayWindowPadding,
      ...
      if (!window->ViewportOwned && viewport_rect.GetWidth() > 0 &&
        ...
        if (window->Viewport->PlatformMonitor == -1) {
          ...
          SetWindowPos(window, g.Viewports[0]->Pos + style.DisplayWindowPadding,
                       ImGuiCond_Always);
        } else {
          ImGuiPlatformMonitor &monitor = g.PlatformIO.Monitors[window->Viewport
            ->PlatformMonitor];
          ClampWindowRect(
              window, ImRect(monitor.WorkPos,
                             monitor.WorkPos + monitor.WorkSize),
              clamp_padding);
        }
      }
    }
    ...
    if (window_just_activated_by_user && !(
      ...
      if (flags & ImGuiWindowFlags_Popup)
        want_focus = true;
      else if ((window->DockIsActive || (flags & ImGuiWindowFlags_ChildWindow)
                == 0) && !(flags & ImGuiWindowFlags_Tooltip))
        want_focus = true;
    }
    ...
    window->InnerRect.Max.x = window->Pos.x + window->Size.x - window->
                              ScrollbarSizes.x;
    window->InnerRect.Max.y = window->Pos.y + window->Size.y - window->
                              ScrollbarSizes.y;
    ...
    window->InnerClipRect.Min.x = ImFloor(
        0.5f + window->InnerRect.Min.x + ImMax(
            ImFloor(window->WindowPadding.x * 0.5f), window->WindowBorderSize));
    window->InnerClipRect.Min.y = ImFloor(
        0.5f + window->InnerRect.Min.y + top_border_size);
    ...
    window->ScrollMax.x = ImMax(
        0.0f, window->ContentSize.x + window->WindowPadding.x * 2.0f - window->
              InnerRect.GetWidth());
    window->ScrollMax.y = ImMax(
        0.0f, window->ContentSize.y + window->WindowPadding.y * 2.0f - window->
              InnerRect.GetHeight());
    ...
    const bool dim_bg_for_modal =
        (flags & ImGuiWindowFlags_Modal) && window == GetTopMostPopupModal() &&
        window->HiddenFramesCannotSkipItems <= 0;
    const bool dim_bg_for_window_list =
        g.NavWindowingTargetAnim && (
          (window == g.NavWindowingTargetAnim->RootWindow) || (
            g.NavWindowingList && (window == g.NavWindowingList) && g.
            NavWindowingList->Viewport != g.NavWindowingTargetAnim->Viewport));
    if (dim_bg_for_modal || dim_bg_for_window_list) {
      const ImU32 dim_bg_col = GetColorU32(
          dim_bg_for_modal
            ? ImGuiCol_ModalWindowDimBg
    ...
    if (is_undocked_or_docked_visible) {
      ...
      if ((flags & ImGuiWindowFlags_ChildWindow) && !(
        ...
        if (window->DrawList->CmdBuffer.back().ElemCount == 0 && parent_window->
            DrawList->VtxBuffer.Size > 0)
          render_decorations_in_parent = true;
      if (render_decorations_in_parent)
        window->DrawList = parent_window->DrawList;

      const ImGuiWindow *window_to_highlight = g.NavWindowingTarget
      ...
      const bool title_bar_is_highlight =
          want_focus || (window_to_highlight && (
                           ...
                           || (window->DockNode && window->DockNode ==
    ...
    const float work_rect_size_x = (window->ContentSizeExplicit.x != 0.0f
                                      ...
                                      : ImMax(
                                          allow_scrollbar_x
                                            ...
                                            : 0.0f,
                                          window->Size.x - window->WindowPadding
                                          .x * 2.0f - window->ScrollbarSizes.x)
    );
    const float work_rect_size_y = (window->ContentSizeExplicit.y != 0.0f
                                      ...
                                      : ImMax(
                                          allow_scrollbar_y
                                            ...
                                            : 0.0f,
                                          window->Size.y - window->WindowPadding
                                          .y * 2.0f - decoration_up_height -
                                          window->ScrollbarSizes.y));
    window->WorkRect.Min.x = ImFloor(
        window->InnerRect.Min.x - window->Scroll.x + ImMax(
            window->WindowPadding.x, window->WindowBorderSize));
    window->WorkRect.Min.y = ImFloor(
        window->InnerRect.Min.y - window->Scroll.y + ImMax(
            window->WindowPadding.y, window->WindowBorderSize));
    ...
    window->ContentsRegionRect.Max.x =
        window->ContentsRegionRect.Min.x + (window->ContentSizeExplicit.x !=
                                            0.0f
                                              ? window->ContentSizeExplicit.x
    ...
    window->ContentsRegionRect.Max.y =
        window->ContentsRegionRect.Min.y + (window->ContentSizeExplicit.y !=
                                            0.0f
                                              ? window->ContentSizeExplicit.y
    ...
    window->DC.CursorStartPos = window->Pos + ImVec2(
                                    window->DC.Indent.x + window->DC.
                                    ColumnsOffset.x,
                                    decoration_up_height + window->WindowPadding
                                    .y - window->Scroll.y);
    ...
    if (p_open != NULL && window->Viewport->PlatformRequestClose && window->
    ...
    */
    ...
    if (g.IO.ConfigFlags & ImGuiConfigFlags_DockingEnable) {
      ...
      if ((g.ActiveId == window->MoveId) && (
        ...
        if ((window->Flags & ImGuiWindowFlags_NoMove) == 0)
          if ((window->RootWindow->Flags & (
                 ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoDocking)) == 0)
    ...
    if (window->DockIsActive) {
      ...
      window->DC.LastItemStatusFlags = IsMouseHoveringRect(
                                           title_bar_rect.Min,
  ...
  if (flags & ImGuiWindowFlags_ChildWindow) {
    ...
    if (!(flags & ImGuiWindowFlags_AlwaysAutoResize) && window->AutoFitFramesX
        <= 0 && window->AutoFitFramesY <= 0)
    ...
    if (parent_window && (parent_window->Collapsed || parent_window->
                          HiddenFramesCanSkipItems > 0))
      window->HiddenFramesCanSkipItems = 1;
    if (parent_window && (parent_window->Collapsed || parent_window->
                          HiddenFramesCannotSkipItems > 0))
  ...
  if (window->Collapsed || !window->Active || window->Hidden)
    if (window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0 && window->
        HiddenFramesCannotSkipItems <= 0)
      skip_items = true;
...
void ImGui::End() {
  ...
  if (window->DockNode && window->DockTabIsVisible)
    if (ImGuiWindow *host_window = window->DockNode->HostWindow) // FIXME-DOCK
      host_window->DC.CursorMaxPos =
          window->DC.CursorMaxPos + window->WindowPadding - host_window->
          WindowPadding;
  ...
  SetCurrentWindow(g.CurrentWindowStack.empty()
                     ? NULL
                     : g.CurrentWindowStack.back());
  if (g.CurrentWindow)
...
void ImGui::BringWindowToDisplayFront(ImGuiWindow *window) {
  ...
  if (current_front_window == window || current_front_window->RootWindow ==
      window)
    return;
...
void ImGui::FocusWindow(ImGuiWindow *window) {
  ...
  if (window->Flags & ImGuiWindowFlags_Popup)
    ...
    if (g.ActiveId != 0 && g.ActiveIdWindow && g.ActiveIdWindow->RootWindow !=
        window)
      ClearActiveID();
...
void ImGui::PushItemWidth(float item_width) {
  ImGuiContext &g = *GImGui;
  ImGuiWindow *window = g.CurrentWindow;
  window->DC.ItemWidth = (item_width == 0.0f
...
void ImGui::PushMultiItemsWidths(int components, float w_full) {
  ...
  const float w_item_one = ImMax(
      1.0f, (float)(int)((w_full - (style.ItemInnerSpacing.x) * (components - 1)
                         ) / (float)components));
...
static const ImGuiStyleVarInfo GStyleVarInfo[] =
{
    ...
    {ImGuiDataType_Float, 2,
     (ImU32)IM_OFFSETOF(ImGuiStyle, SelectableTextAlign)},
...
void ImGui::PopStyleVar(int count) {
  ...
  while (count > 0) {
    ...
    if (info->Type == ImGuiDataType_Float && info->Count == 1) {
      ...
      if (info->Type == ImGuiDataType_Float && info->Count == 2) {
        ((float *)data)[0] = backup.BackupFloat[0];
...
bool ImGui::IsWindowHovered(ImGuiHoveredFlags flags) {
  ...
  if (flags & ImGuiHoveredFlags_AnyWindow) {
    ...
    case ImGuiHoveredFlags_RootWindow | ImGuiHoveredFlags_ChildWindows:
      if (g.HoveredWindow == NULL || g.HoveredWindow->RootWindowDockStop != g.
          CurrentWindow->RootWindowDockStop)
        return false;
      break;
    ...
    case ImGuiHoveredFlags_ChildWindows:
      if (g.HoveredWindow == NULL || !IsWindowChildOf(
  ...
  if (!IsWindowContentHoverable(g.HoveredWindow, flags))
    return false;
  if (!(flags & ImGuiHoveredFlags_AllowWhenBlockedByActiveItem))
    if (g.ActiveId != 0 && !g.ActiveIdAllowOverlap && g.ActiveId != g.
        HoveredWindow->MoveId)
      return false;
  return true;
}
...
bool ImGui::IsWindowFocused(ImGuiFocusedFlags flags) {
  ...
  case ImGuiFocusedFlags_RootWindow | ImGuiFocusedFlags_ChildWindows:
    return g.NavWindow && g.NavWindow->RootWindowDockStop == g.CurrentWindow->
           RootWindowDockStop;
...
static void SetWindowHitTestHole(ImGuiWindow *window, const ImVec2 &pos,
  ...
  window->HitTestHoleOffset = ImVec2ih((short)(pos.x - window->Pos.x),
                                       (short)(pos.y - window->Pos.y));
}
...
void ImGui::SetNextWindowPosCenter(ImGuiCond cond) {
  ...
  SetNextWindowPos(viewport->Pos + viewport->Size * 0.5f, cond,
                   ImVec2(0.5f, 0.5f));
...
ImGuiViewport *ImGui::GetWindowViewport() {
  ImGuiContext &g = *GImGui;
  IM_ASSERT(
      g.CurrentViewport != NULL && g.CurrentViewport == g.CurrentWindow->
      Viewport);
...
void ImGui::SetScrollHereX(float center_x_ratio) {
  ImGuiWindow *window = GetCurrentWindow();
  float target_x = window->DC.LastItemRect.Min.x - window->Pos.x;
  ...
  target_x += (last_item_width * center_x_ratio) + (
    GImGui->Style.ItemSpacing.x * (center_x_ratio - 0.5f) * 2.0f);
...
void ImGui::SetScrollHereY(float center_y_ratio) {
  ...
  target_y += (window->DC.PrevLineSize.y * center_y_ratio) + (
    GImGui->Style.ItemSpacing.y * (center_y_ratio - 0.5f) * 2.0f);
...
void ImGui::SetItemDefaultFocus() {
  ...
  if (g.NavWindow == window->RootWindowForNav && (
        g.NavInitRequest || g.NavInitResultId != 0) && g.NavLayer == g.NavWindow
      ->DC.NavLayerCurrent) {
    g.NavInitRequest = false;
...
void ImGui::BeginGroup() {
  ...
  group_data.BackupActiveIdPreviousFrameIsAlive = g.
      ActiveIdPreviousFrameIsAlive;
  group_data.EmitItem = true;

  window->DC.GroupOffset.x = window->DC.CursorPos.x - window->Pos.x - window->DC
                             .ColumnsOffset.x;
...
void ImGui::SameLine(float offset_from_start_x, float spacing_w) {
  ...
  if (offset_from_start_x != 0.0f) {
    ...
    window->DC.CursorPos.x = window->Pos.x - window->Scroll.x +
                             offset_from_start_x + spacing_w + window->DC.
...
void ImGui::Indent(float indent_w) {
  ...
  window->DC.CursorPos.x = window->Pos.x + window->DC.Indent.x + window->DC.
                           ColumnsOffset.x;
}
...
bool ImGui::IsPopupOpen(ImGuiID id) {
  ImGuiContext &g = *GImGui;
  return g.OpenPopupStack.Size > g.BeginPopupStack.Size && g.OpenPopupStack[g.
           BeginPopupStack.Size].PopupId == id;
}

bool ImGui::IsPopupOpen(const char *str_id) {
  ImGuiContext &g = *GImGui;
  return g.OpenPopupStack.Size > g.BeginPopupStack.Size && g.OpenPopupStack[g.
           BeginPopupStack.Size].PopupId == g.CurrentWindow->GetID(str_id);
}
...
void ImGui::OpenPopupEx(ImGuiID id) {
  ...
  if (g.OpenPopupStack.Size < current_stack_size + 1) {
    ...
    if (g.OpenPopupStack[current_stack_size].PopupId == id && g.OpenPopupStack[
          current_stack_size].OpenFrameCount == g.FrameCount - 1) {
      g.OpenPopupStack[current_stack_size].OpenFrameCount = popup_ref.
          OpenFrameCount;
    } else {
...
void ImGui::CloseCurrentPopup() {
  ...
  if (popup_idx < 0 || popup_idx >= g.OpenPopupStack.Size || g.BeginPopupStack[
        popup_idx].PopupId != g.OpenPopupStack[popup_idx].PopupId)
    return;
...
bool ImGui::BeginPopupModal(const char *name, bool *p_open,
  ...
  if ((g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasPos) == 0)
    SetNextWindowPos(window->Viewport->GetCenter(), ImGuiCond_Appearing,
                     ImVec2(0.5f, 0.5f));
...
ImRect ImGui::GetWindowAllowedExtentRect(ImGuiWindow *window) {
  ...
  if (window->ViewportAllowPlatformMonitorExtend >= 0) {
    ...
    const ImGuiPlatformMonitor &monitor = g.PlatformIO.Monitors[window->
...
static bool NavScoreItem(ImGuiNavMoveResult *result, ImRect cand) {
  ...
  if (dbx != 0.0f || dby != 0.0f) {
    ...
    quadrant = (window->DC.LastItemId < g.NavId)
                 ? ImGuiDir_Left
                 : ImGuiDir_Right;
  }
...
static void ImGui::NavProcessItem(ImGuiWindow *window, const ImRect &nav_bb,
  ...
  if (g.NavInitRequest && g.NavLayer == window->DC.NavLayerCurrent) {
    ...
    if (!(item_flags & ImGuiItemFlags_NoNavDefaultFocus) || g.NavInitResultId ==
        0) {
  ...
  if ((g.NavId != id || (g.NavMoveRequestFlags &
    ...
    if ((g.NavMoveRequestFlags & ImGuiNavMoveFlags_AlsoScoreVisibleSet) &&
      ...
      if (ImClamp(nav_bb.Max.y, window->ClipRect.Min.y, window->ClipRect.Max.y)
          - ImClamp(nav_bb.Min.y, window->ClipRect.Min.y,
                    window->ClipRect.Max.y) >= (nav_bb.Max.y - nav_bb.Min.y) *
          VISIBLE_RATIO)
...
bool ImGui::NavMoveRequestButNoResultYet() {
  ImGuiContext &g = *GImGui;
  return g.NavMoveRequest && g.NavMoveResultLocal.ID == 0 && g.
         NavMoveResultOther.ID == 0;
}
...
void ImGui::NavMoveRequestTryWrapping(ImGuiWindow *window,
                                      ImGuiNavMoveFlags move_flags) {
  ImGuiContext &g = *GImGui;
  if (g.NavWindow != window || !NavMoveRequestButNoResultYet() || g.
      NavMoveRequestForward != ImGuiNavForward_None || g.NavLayer != 0)
    return;
  ...
  if (g.NavMoveDir == ImGuiDir_Left && (
    ...
    bb_rel.Min.x = bb_rel.Max.x = ImMax(window->SizeFull.x,
                                        window->ContentSize.x + window->
                                        WindowPadding.x * 2.0f) - window->Scroll
                                  .x;
  ...
  if (g.NavMoveDir == ImGuiDir_Up && (
    ...
    bb_rel.Min.y = bb_rel.Max.y = ImMax(window->SizeFull.y,
                                        window->ContentSize.y + window->
                                        WindowPadding.y * 2.0f) - window->Scroll
                                  .y;
...
ImGui::NavSaveLastChildNavWindowIntoParent(ImGuiWindow *nav_window) {
  ...
  while (parent_window && (parent_window->Flags & ImGuiWindowFlags_ChildWindow)
         != 0 && (parent_window->Flags & (
...
static void NavRestoreLayer(ImGuiNavLayer layer) {
  ...
  if (g.NavWindow->NavLastIds[layer] != 0)
    ImGui::SetNavIDWithRectRel(g.NavWindow->NavLastIds[layer], layer,
...
static ImVec2 ImGui::NavCalcPreferredRefPos() {
  ...
  if (g.NavDisableHighlight || !g.NavDisableMouseHover || !g.NavWindow) {
    ...
    ImVec2 pos = g.NavWindow->Pos + ImVec2(
                     rect_rel.Min.x + ImMin(g.Style.FramePadding.x * 4,
                                            rect_rel.GetWidth()),
...
ImVec2 ImGui::GetNavInputAmount2d(ImGuiNavDirSourceFlags dir_sources,
  ...
  if (dir_sources & ImGuiNavDirSourceFlags_Keyboard)
    delta += ImVec2(
        ...
        GetNavInputAmount(ImGuiNavInput_KeyDown_, mode) - GetNavInputAmount(
  ...
  if (dir_sources & ImGuiNavDirSourceFlags_PadDPad)
    delta += ImVec2(
        ...
        GetNavInputAmount(ImGuiNavInput_DpadDown, mode) - GetNavInputAmount(
  ...
  if (dir_sources & ImGuiNavDirSourceFlags_PadLStick)
    delta += ImVec2(
        ...
        GetNavInputAmount(ImGuiNavInput_LStickDown, mode) - GetNavInputAmount(
...
static void NavScrollToBringItemIntoView(ImGuiWindow *window,
  ...
  if (window->ScrollbarX && item_rect.Min.x < window_rect.Min.x) {
    window->ScrollTarget.x = item_rect.Min.x - window->Pos.x + window->Scroll.x
                             - g.Style.ItemSpacing.x;
    window->ScrollTargetCenterRatio.x = 0.0f;
  } else if (window->ScrollbarX && item_rect.Max.x >= window_rect.Max.x) {
    window->ScrollTarget.x = item_rect.Max.x - window->Pos.x + window->Scroll.x
                             + g.Style.ItemSpacing.x;
    window->ScrollTargetCenterRatio.x = 1.0f;
  }
  if (item_rect.Min.y < window_rect.Min.y) {
    window->ScrollTarget.y = item_rect.Min.y - window->Pos.y + window->Scroll.y
                             - g.Style.ItemSpacing.y;
    window->ScrollTargetCenterRatio.y = 0.0f;
  } else if (item_rect.Max.y >= window_rect.Max.y) {
    window->ScrollTarget.y = item_rect.Max.y - window->Pos.y + window->Scroll.y
                             + g.Style.ItemSpacing.y;
...
static void ImGui::NavUpdate() {
  ...
  bool nav_keyboard_active = (g.IO.ConfigFlags &
                              ImGuiConfigFlags_NavEnableKeyboard) != 0;
  bool nav_gamepad_active = (g.IO.ConfigFlags &
                             ImGuiConfigFlags_NavEnableGamepad) != 0 && (
                              g.IO.BackendFlags & ImGuiBackendFlags_HasGamepad)
                            != 0;
  if (nav_gamepad_active)
    if (g.IO.NavInputs[ImGuiNavInput_Activate] > 0.0f || g.IO.NavInputs[
          ImGuiNavInput_Input] > 0.0f || g.IO.NavInputs[ImGuiNavInput_Cancel] >
        0.0f || g.IO.NavInputs[ImGuiNavInput_Menu] > 0.0f)
...
#undef NAV_MAP_KEY
  ...
  for (int i = 0; i < IM_ARRAYSIZE(g.IO.NavInputs); i++)
    g.IO.NavInputsDownDuration[i] = (g.IO.NavInputs[i] > 0.0f)
                                      ? (g.IO.NavInputsDownDuration[i] < 0.0f
                                           ? 0.0f
                                           : g.IO.NavInputsDownDuration[i] + g.
                                             IO.DeltaTime)
                                      : -1.0f;
  ...
  if (g.NavWindow && g.NavWindow->NavLastChildNavWindow != NULL && g.NavLayer ==
      0)
    ...
    if (g.ActiveId != 0) {
      if (!(g.ActiveIdBlockNavInputFlags & (1 << ImGuiNavInput_Cancel)))
        ClearActiveID();
               ...
               ) && !(g.NavWindow->Flags & ImGuiWindowFlags_Popup) && g.
               NavWindow->ParentWindow && g.NavWindow != g.NavWindow->
               RootWindowDockStop) {
      // Exit child window
  ...
  if (g.NavId != 0 && !g.NavDisableHighlight && !g.NavWindowingTarget && g.
      NavWindow && !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs)) {
    bool activate_down = IsNavInputDown(ImGuiNavInput_Activate);
    bool activate_pressed = activate_down && IsNavInputPressed(
  ...
  const int allowed_dir_flags = (g.ActiveId == 0)
                                  ? ~0
  ...
  // Scrolling
  if (g.NavWindow && !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) && !g.
      NavWindowingTarget) {
    // *Fallback* manual-scroll with Nav directional keys when window has no navigable item
    ImGuiWindow *window = g.NavWindow;
    const float scroll_speed =
        ImFloor(window->CalcFontSize() * 100 * g.IO.DeltaTime + 0.5f);
    // We need round the scrolling speed because sub-pixel scroll isn't reliably supported.
    if (window->DC.NavLayerActiveMask == 0x00 && window->DC.NavHasScroll && g.
        NavMoveRequest) {
      if (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right)
        SetWindowScrollX(window, ImFloor(
                             window->Scroll.x + ((g.NavMoveDir == ImGuiDir_Left)
                                                   ? -1.0f
                                                   : +1.0f) * scroll_speed));
      if (g.NavMoveDir == ImGuiDir_Up || g.NavMoveDir == ImGuiDir_Down)
        SetWindowScrollY(window, ImFloor(
                             window->Scroll.y + ((g.NavMoveDir == ImGuiDir_Up)
                                                   ? -1.0f
                                                   : +1.0f) * scroll_speed));
    }
...
static void ImGui::NavUpdateMoveResult() {
  ...
  if (g.NavMoveRequestFlags & ImGuiNavMoveFlags_AlsoScoreVisibleSet)
    if (g.NavMoveResultLocalVisibleSet.ID != 0 && g.NavMoveResultLocalVisibleSet
                                                   .ID != g.NavId)
      result = &g.NavMoveResultLocalVisibleSet;

  // Maybe entering a flattened child from the outside? In this case solve the tie using the regular scoring rules.
  if (result != &g.NavMoveResultOther && g.NavMoveResultOther.ID != 0 && g.
      NavMoveResultOther.Window->ParentWindow == g.NavWindow)
    if ((g.NavMoveResultOther.DistBox < result->DistBox) || (
          g.NavMoveResultOther.DistBox == result->DistBox && g.
...
static float ImGui::NavUpdatePageUpPageDown(int allowed_dir_flags) {
  ...
  if (g.NavMoveDir == ImGuiDir_None && g.NavWindow && !(
        g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) && !g.
      NavWindowingTarget && g.NavLayer == 0) {
    ImGuiWindow *window = g.NavWindow;
...
static void ImGui::NavUpdateWindowing() {
  ...
  bool start_windowing_with_gamepad =
      !g.NavWindowingTarget && IsNavInputPressed(
  ...
  if (start_windowing_with_gamepad || start_windowing_with_keyboard)
    if (ImGuiWindow *window = g.NavWindow
                                ...
                                : FindWindowNavFocusable(
                                    g.WindowsFocusOrder.Size - 1, -INT_MAX,
                                    -1)) {
  ...
  if (g.NavWindowingTarget && g.NavInputSource == ImGuiInputSource_NavGamepad) {
    ...
    const int focus_change_dir = (int)IsNavInputPressed(
                                     ImGuiNavInput_FocusPrev,
                                     ImGuiInputReadMode_RepeatSlow) - (int)
                                 IsNavInputPressed(
  ...
  // Move window
  if (g.NavWindowingTarget && !(g.NavWindowingTarget->Flags &
                                ImGuiWindowFlags_NoMove)) {
    ImVec2 move_delta;
    if (g.NavInputSource == ImGuiInputSource_NavKeyboard && !g.IO.KeyShift)
    ...
    if (move_delta.x != 0.0f || move_delta.y != 0.0f) {
      ...
      const float move_speed = ImFloor(
          NAV_MOVE_SPEED * g.IO.DeltaTime * ImMin(
      ...
      SetWindowPos(g.NavWindowingTarget->RootWindow,
                   g.NavWindowingTarget->RootWindow->Pos + move_delta *
                   move_speed, ImGuiCond_Always);
      g.NavDisableMouseHover = true;
  ...
  // Apply final focus
  if (apply_focus_window && (g.NavWindow == NULL || apply_focus_window != g.
                             NavWindow->RootWindowDockStop)) {
    ImGuiViewport *previous_viewport = g.NavWindow
    ...
    if (apply_focus_window->Viewport != previous_viewport && g.PlatformIO.
  ...
  if (apply_toggle_layer && g.NavWindow) {
    ...
    const ImGuiNavLayer new_nav_layer = (g.NavWindow->DC.NavLayerActiveMask & (
...
static const char *GetFallbackWindowNameForWindowingList(ImGuiWindow *window) {
  if (window->Flags & ImGuiWindowFlags_Popup)
    return "(Popup)";
  if ((window->Flags & ImGuiWindowFlags_MenuBar) && strcmp(
          window->Name, "##MainMenuBar") == 0)
...
void ImGui::NavUpdateWindowingList() {
  ...
  ImGuiViewportP *viewport = /*g.NavWindow ? g.NavWindow->Viewport :*/ (
    ImGuiViewportP *)GetMainViewport();
  SetNextWindowSizeConstraints(
      ImVec2(viewport->Size.x * 0.20f, viewport->Size.y * 0.20f),
      ImVec2(FLT_MAX, FLT_MAX));
  SetNextWindowPos(viewport->Pos + viewport->Size * 0.5f, ImGuiCond_Always,
                   ImVec2(0.5f, 0.5f));
...
void ImGui::NextColumn() {
  ...
  if (++columns->Current < columns->Count) {
    ...
    window->DC.ColumnsOffset.x =
        GetColumnOffset(columns->Current) - window->DC.Indent.x + g.Style.
        ItemSpacing.x;
...
static float
OffsetNormToPixels(const ImGuiColumns *columns, float offset_norm) {
...
static float GetDraggedColumnOffset(ImGuiColumns *columns, int column_index) {
  ...
  if ((columns->Flags & ImGuiColumnsFlags_NoPreserveWidths))
    x = ImMin(
        x, ImGui::GetColumnOffset(column_index + 1) - g.Style.
           ColumnsMinSpacing);
...
static float GetColumnWidthEx(ImGuiColumns *columns, int column_index,
  ...
  if (before_resize)
    ...
    offset_norm = columns->Columns[column_index + 1].OffsetNorm - columns->
...
void ImGui::SetColumnOffset(int column_index, float offset) {
  ...
  columns->Columns[column_index].OffsetNorm = PixelsToOffsetNorm(
...
void ImGui::BeginColumns(const char *str_id, int columns_count,
  ...
  columns->OffMaxX = ImMax(window->WorkRect.Max.x - window->Pos.x,
  ...
  for (int n = 0; n < columns_count; n++) {
    ...
    float clip_x2 = ImFloor(
        0.5f + window->Pos.x + GetColumnOffset(n + 1) - 1.0f);
    ...
    if (dragging_column != -1) {
      if (!columns->IsBeingResized)
        for (int n = 0; n < columns->Count + 1; n++)
          columns->Columns[n].OffsetNormBeforeResize = columns->Columns[n].
              OffsetNorm;
...
void ImGui::Columns(int columns_count, const char *id, bool border) {
  ...
  ImGuiColumns *columns = window->DC.CurrentColumns;
  if (columns != NULL && columns->Count == columns_count && columns->Flags ==
      flags)
    return;
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags) {
  ...
  if (!(flags & ImGuiDragDropFlags_SourceExtern)) {
    ...
    if (source_id == 0) {
      ...
      // Early out
      if ((window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_HoveredRect) ==
          0 && (g.ActiveId == 0 || g.ActiveIdWindow != window))
        return false;
      ...
      source_id = window->DC.LastItemId = window->GetIDFromRectangle(
...
bool ImGui::BeginDragDropTargetCustom(const ImRect &bb, ImGuiID id) {
  ...
  if (g.HoveredWindowUnderMovingWindow == NULL || window->RootWindow != g.
      HoveredWindowUnderMovingWindow->RootWindow)
    return false;
  IM_ASSERT(id != 0);
...
const ImGuiPayload *ImGui::AcceptDragDropPayload(
  ...
  if (!(flags & ImGuiDragDropFlags_AcceptNoDrawDefaultRect) && payload.Preview
    ...
    if (push_clip_rect)
      window->DrawList->
              PushClipRect(r.Min - ImVec2(1, 1), r.Max + ImVec2(1, 1));
...
void ImGui::LogBegin(ImGuiLogType type, int auto_open_depth) {
  ...
  g.LogType = type;
  g.LogDepthRef = window->DC.TreeDepth;
  g.LogDepthToExpand = ((auto_open_depth >= 0)
                          ? auto_open_depth
...
static void SettingsHandlerWindow_ReadLine(ImGuiContext *ctx,
  ...
  if (sscanf(line, "Pos=%f,%f", &x, &y) == 2
  ) { settings->Pos = ImVec2(x, y); } else if (
    sscanf(line, "Size=%f,%f", &x, &y) == 2
  ) { settings->Size = ImMax(ImVec2(x, y), g.Style.WindowMinSize); } else if (
    sscanf(line, "ViewportId=0x%08X", &u1) == 1
  ) { settings->ViewportId = u1; } else if (
    sscanf(line, "ViewportPos=%f,%f", &x, &y) == 2
  ) { settings->ViewportPos = ImVec2(x, y); } else if (
    sscanf(line, "Collapsed=%d", &i) == 1
  ) { settings->Collapsed = (i != 0); } else if (
    sscanf(line, "DockId=0x%X,%d", &u1, &i) == 2) {
    settings->DockId = u1;
    ...
    if (sscanf(line, "ClassId=0x%X", &u1) == 1) { settings->ClassId = u1; }
}
...
static void SettingsHandlerWindow_WriteAll(ImGuiContext *ctx,
  ...
  for (int i = 0; i != g.Windows.Size; i++) {
    ...
    IM_ASSERT(
        window->DockNode == NULL || window->DockNode->ID == window->DockId);
  ...
  for (int i = 0; i != g.SettingsWindows.Size; i++) {
    ...
    if (settings->ViewportId != 0 && settings->ViewportId !=
        ImGui::IMGUI_VIEWPORT_DEFAULT_ID) {
      buf->appendf("ViewportPos=%d,%d\n", (int)settings->ViewportPos.x,
                   (int)settings->ViewportPos.y);
      buf->appendf("ViewportId=0x%08X\n", settings->ViewportId);
    }
    if (settings->Pos.x != 0.0f || settings->Pos.y != 0.0f || settings->
        ViewportId == ImGui::IMGUI_VIEWPORT_DEFAULT_ID)
...
static bool ImGui::GetWindowAlwaysWantOwnViewport(ImGuiWindow *window) {
  ...
  if (g.IO.ConfigViewportsNoAutoMerge && (
        g.ConfigFlagsForFrame & ImGuiConfigFlags_ViewportsEnable))
    if (!window->DockIsActive)
      if ((window->Flags & (ImGuiWindowFlags_ChildWindow |
                            ImGuiWindowFlags_ChildMenu |
                            ImGuiWindowFlags_Tooltip | ImGuiWindowFlags_Popup))
          == 0)
        return true;
  return false;
}
...
static bool ImGui::UpdateTryMergeWindowIntoHostViewport(
  ...
  if (!(viewport->Flags & (ImGuiViewportFlags_CanHostOtherWindows |
                           ImGuiViewportFlags_Minimized)) || window->Viewport ==
      viewport)
    return false;
...
static ImGuiViewportP *FindViewportHoveredFromPlatformWindowStack(
  ...
  for (int n = 0; n < g.Viewports.Size; n++) {
    ...
    if (!(viewport->Flags & (ImGuiViewportFlags_NoInputs |
                             ImGuiViewportFlags_Minimized)) && viewport->
        GetRect().Contains(mouse_platform_pos))
...
static void ImGui::UpdateViewportsNewFrame() {
  ...
  if (g.ConfigFlagsForFrame & ImGuiConfigFlags_ViewportsEnable)
    main_viewport_platform_pos = (main_viewport->Flags &
                                  ImGuiViewportFlags_Minimized)
                                   ? main_viewport->Pos
                                   : g.PlatformIO.Platform_GetWindowPos(
                                       main_viewport);
  AddUpdateViewport(NULL, IMGUI_VIEWPORT_DEFAULT_ID, main_viewport_platform_pos,
                    main_viewport_platform_size,
  ...
  for (int n = 0; n < g.Viewports.Size; n++) {
    ...
    if ((g.ConfigFlagsForFrame & ImGuiConfigFlags_ViewportsEnable)) {
      ...
      if (!(viewport->Flags & ImGuiViewportFlags_Minimized) &&
        ...
        if (viewport->PlatformRequestMove)
          viewport->Pos = viewport->LastPlatformPos = g.PlatformIO.
                          Platform_GetWindowPos(viewport);
        if (viewport->PlatformRequestResize)
          viewport->Size = viewport->LastPlatformSize = g.PlatformIO.
                           Platform_GetWindowSize(viewport);
      }
    }
    ...
    if ((viewport->Flags & ImGuiViewportFlags_CanHostOtherWindows) && (
          viewport_delta.x != 0.0f || viewport_delta.y != 0.0f))
      for (int window_n = 0; window_n < g.Windows.Size; window_n++)
        if (g.Windows[window_n]->Viewport == viewport || (
              g.ConfigFlagsForFrame & ImGuiConfigFlags_ViewportsEnable) == 0)
  ...
  if (g.IO.BackendFlags & ImGuiBackendFlags_HasMouseHoveredViewport) {
    ...
    viewport_hovered =
        FindViewportHoveredFromPlatformWindowStack(g.IO.MousePos);
  }
...
ImGuiViewportP *ImGui::AddUpdateViewport(ImGuiWindow *window, ImGuiID id,
  ...
  if (viewport) {
    ...
    if (viewport->PlatformMonitor != -1)
      viewport->DpiScale = g.PlatformIO.Monitors[viewport->PlatformMonitor].
          DpiScale;
  }
...
static void ImGui::UpdateSelectWindowViewport(ImGuiWindow *window) {
  ...
  if ((flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_Tooltip)) && window->
      Appearing) {
  ...
  if ((g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasViewport) == 0) {
    ...
    if (window->Viewport == NULL && window->ViewportId != 0) {
      ...
      if (window->Viewport == NULL && window->ViewportPos.x != FLT_MAX && window
          ->ViewportPos.y != FLT_MAX)
        window->Viewport = AddUpdateViewport(window, window->ID,
                                             window->ViewportPos, window->Size,
  ...
  if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasViewport) {
    ...
    window->Viewport = (ImGuiViewportP *)FindViewportByID(
        g.NextWindowData.ViewportId);
    window->ViewportId = g.NextWindowData.ViewportId;
    // Store ID even if Viewport isn't resolved yet.
    lock_viewport = true;
  ...
  } else if (g.MovingWindow && g.MovingWindow->RootWindow == window &&
             IsMousePosValid()) {
  ...
  if (!lock_viewport) {
    if (flags & (ImGuiWindowFlags_Tooltip | ImGuiWindowFlags_Popup)) {
      ...
      bool use_mouse_ref = (g.NavDisableHighlight || !g.NavDisableMouseHover ||
                            !g.NavWindow);
      ...
      if ((window->Appearing || (flags & ImGuiWindowFlags_Tooltip)) && (
        ...
        window->ViewportAllowPlatformMonitorExtend = window->Viewport->
            PlatformMonitor;
    } else if (window->Viewport && window != window->Viewport->Window && window
               ->Viewport->Window && !(flags & ImGuiWindowFlags_ChildWindow)) {
      // When called from Begin() we don't have access to a proper version of the Hidden flag yet, so we replicate this code.
      const bool will_be_visible = (window->DockIsActive && !window->
                                    DockTabIsVisible)
                                     ? false
                                     : true;
      if ((window->Flags & ImGuiWindowFlags_DockNodeHost) && window->Viewport->
          LastFrameActive < g.FrameCount && will_be_visible) {
...
void ImGui::UpdatePlatformWindows() {
  ...
  for (int i = 1; i < g.Viewports.Size; i++) {
    ...
    destroy_platform_window |= (viewport->Window && !IsWindowActiveAndVisible(
                                    viewport->Window));
    ...
    if ((viewport->LastPlatformPos.x != viewport->Pos.x || viewport->
         LastPlatformPos.y != viewport->Pos.y) && !viewport->PlatformRequestMove
    )
      g.PlatformIO.Platform_SetWindowPos(viewport, viewport->Pos);
    if ((viewport->LastPlatformSize.x != viewport->Size.x || viewport->
         LastPlatformSize.y != viewport->Size.y) && !viewport->
        PlatformRequestResize)
      g.PlatformIO.Platform_SetWindowSize(viewport, viewport->Size);
    if ((viewport->LastRendererSize.x != viewport->Size.x || viewport->
         LastRendererSize.y != viewport->Size.y) && g.PlatformIO.
    ...
    if (ImGuiWindow *window_for_title = GetWindowForTitleDisplay(
        viewport->Window)) {
      ...
      if (viewport->LastNameHash != title_hash) {
        ...
        *title_end = 0;
        // Cut existing buffer short instead of doing an alloc/free, no small gain.
    ...
    if (viewport->LastAlpha != viewport->Alpha && g.PlatformIO.
  ...
  if (g.PlatformIO.Platform_GetWindowFocus != NULL) {
    ...
    if (focused_viewport && g.PlatformLastFocusedViewport != focused_viewport->
        ID) {
      if (focused_viewport->LastFrontMostStampCount != g.
          ViewportFrontMostStampCount)
        focused_viewport->LastFrontMostStampCount = ++g.
            ViewportFrontMostStampCount;
...
static int ImGui::FindPlatformMonitorForRect(const ImRect &rect) {
  ...
  const float surface_threshold = ImMax(
      rect.GetWidth() * rect.GetHeight() * 0.5f, 1.0f);
  int best_monitor_n = -1;
  float best_monitor_surface = 0.001f;

  for (int monitor_n = 0; monitor_n < g.PlatformIO.Monitors.Size &&
                          best_monitor_surface < surface_threshold; monitor_n++
  ) {
    const ImGuiPlatformMonitor &monitor = g.PlatformIO.Monitors[monitor_n];
    const ImRect monitor_rect = ImRect(monitor.MainPos,
                                       monitor.MainPos + monitor.MainSize);
    if (monitor_rect.Contains(rect))
      return monitor_n;
    ImRect overlapping_rect = rect;
    overlapping_rect.ClipWithFull(monitor_rect);
    float overlapping_surface = overlapping_rect.GetWidth() * overlapping_rect.
                                GetHeight();
...
void ImGui::DestroyPlatformWindow(ImGuiViewportP *viewport) {
  ...
  if (viewport->PlatformWindowCreated) {
    ...
    IM_ASSERT(
        viewport->RendererUserData == NULL && viewport->PlatformUserData ==
        NULL);
...
struct ImGuiDockPreviewData {
  ...
  ImGuiDockPreviewData()
    : FutureNode(0) {
    IsDropAllowed = IsCenterAvailable =
                    IsSidesAvailable = IsSplitDirExplicit = false;
    SplitNode = NULL;
...
static void DockContextProcessUndockWindow(ImGuiContext *ctx,
                                           ...
                                           bool clear_persistent_docking_ref =
                                               true);
static void
DockContextProcessUndockNode(ImGuiContext *ctx, ImGuiDockNode *node);
...
static void DockContextClearNodes(ImGuiContext *ctx, ImGuiID root_id,
                                  bool clear_persistent_docking_refs);
...
static void DockNodePreviewDockCalc(ImGuiWindow *host_window,
...
DockNodeTreeFindNodeByPos(ImGuiDockNode *node, ImVec2 pos);
...
void ImGui::DockContextOnLoadSettings(ImGuiContext *ctx) {
  ...
  DockContextBuildNodesFromSettings(ctx, dc->SettingsNodes.Data,
                                    dc->SettingsNodes.Size);
}

void ImGui::DockContextClearNodes(ImGuiContext *ctx, ImGuiID root_id,
                                  bool clear_persistent_docking_references) {
  IM_UNUSED(ctx);
  IM_ASSERT(ctx == GImGui);
  DockBuilderRemoveNodeDockedWindows(root_id,
                                     clear_persistent_docking_references);
...
static int IMGUI_CDECL DockNodeComparerDepthMostFirst(
...
static void ImGui::DockContextPruneUnusedSettingsNodes(ImGuiContext *ctx) {
  ...
  for (int settings_n = 0; settings_n < dc->SettingsNodes.Size; settings_n++) {
    ...
    ImGuiDockContextPruneNodeData *parent_data = settings->ParentID
                                                   ? pool.GetByKey(
                                                       settings->ParentID)
                                                   : 0;
  ...
  for (int settings_n = 0; settings_n < g.SettingsWindows.Size; settings_n++)
    if (ImGuiID dock_id = g.SettingsWindows[settings_n].DockId)
      if (ImGuiDockContextPruneNodeData *data = pool.GetByKey(dock_id)) {
        ImGuiDockContextPruneNodeData *data_root = (data->RootID == dock_id)
                                                     ? data
                                                     : pool.GetByKey(
                                                         data->RootID);
        data->CountWindows++;
  ...
  for (int settings_n = 0; settings_n < dc->SettingsNodes.Size; settings_n++) {
    ...
    if (data->CountWindows > 1)
      continue;
    ImGuiDockContextPruneNodeData *data_root = (data->RootID == settings->ID)
                                                 ? data
                                                 : pool.GetByKey(data->RootID);

    bool remove = false;
    remove |= (data->CountWindows == 1 && settings->ParentID == 0 && data->
               CountChildNodes == 0 && !(
                 settings->Flags & ImGuiDockNodeFlags_CentralNode));
    // Floating root node with only 1 window
    remove |= (data->CountWindows == 0 && settings->ParentID == 0 && data->
               CountChildNodes == 0); // Leaf nodes with 0 window
...
static void ImGui::DockContextBuildNodesFromSettings(
  ...
  for (int node_n = 0; node_n < node_settings_count; node_n++) {
    ...
    node->ParentNode = settings->ParentID
                         ? DockContextFindNodeByID(ctx, settings->ParentID)
                         : NULL;
    node->Pos = ImVec2(settings->Pos.x, settings->Pos.y);
    node->Size = ImVec2(settings->Size.x, settings->Size.y);
    node->SizeRef = ImVec2(settings->SizeRef.x, settings->SizeRef.y);
    node->AuthorityForPos = node->AuthorityForSize =
                            node->AuthorityForViewport =
    ...
    node->HostWindow = FindWindowByName(
        DockNodeGetHostWindowTitle(root_node, host_window_title,
...
void ImGui::DockContextProcessDock(ImGuiContext *ctx, ImGuiDockRequest *req) {
  ...
  if (split_dir != ImGuiDir_None) {
    ...
    const ImGuiAxis split_axis = (split_dir == ImGuiDir_Left || split_dir ==
                                  ImGuiDir_Right)
                                   ? ImGuiAxis_X
                                   : ImGuiAxis_Y;
    ...
    if (payload_node != NULL) {
      ...
      if (payload_node->IsSplitNode()) {
        if (node->Windows.Size > 0) {
          ...
          // The docking should have been blocked by DockNodePreviewDockCalc() early on and never submitted.
        ...
        if (node->IsCentralNode()) {
          ...
          ImGuiDockNode *last_focused_root_node = DockNodeGetRootNode(
              last_focused_node);
          IM_ASSERT(
...
void ImGui::DockContextProcessUndockNode(ImGuiContext *ctx,
  ...
  if (node->IsRootNode() || node->IsCentralNode()) {
    ...
    new_node->AuthorityForPos = new_node->AuthorityForSize =
    ...
    IM_ASSERT(
        node->ParentNode->ChildNodes[0] == node || node->ParentNode->ChildNodes[
          1] == node);
...
bool ImGui::DockContextCalcDropPosForDocking(ImGuiWindow *target,
  ...
  if (split_outer) {
    ...
    DockNodePreviewDockCalc(target, target_node, payload, &split_data, false,
                            split_outer);
...
ImGuiDockNode::ImGuiDockNode(ImGuiID id) {
  ...
  WantCloseAll = WantLockSizeOnce =
                 WantMouseMove =
                 WantHiddenTabBarUpdate = WantHiddenTabBarToggle = false;
}
...
static void ImGui::DockNodeAddWindow(ImGuiDockNode *node, ImGuiWindow *window,
  ...
  if (node->HostWindow == NULL && node->Windows.Size == 2 && node->Windows[0]->
      WasActive == false) {
...
static void ImGui::DockNodeRemoveWindow(ImGuiDockNode *node,
  ...
  UpdateWindowParentAndRootLinks(
      window, window->Flags & ~ImGuiWindowFlags_ChildWindow, NULL);
  // Update immediately
...
static void ImGui::DockNodeMoveWindows(ImGuiDockNode *dst_node,
  ...
  for (int n = 0; n < src_node->Windows.Size; n++) {
    ImGuiWindow *window = src_tab_bar
                            ? src_tab_bar->Tabs[n].Window
                            : src_node->Windows[n];
    window->DockNode = NULL;
    window->DockIsActive = false;
...
static void ImGui::DockNodeUpdateVisibleFlagAndInactiveChilds(
  ...
  for (int window_n = 0; window_n < node->Windows.Size; window_n++) {
    ...
    remove |= node_was_active && (
          node->WantCloseAll || node->WantCloseTabID == window->ID) && window->
  ...
  if (node->WantHiddenTabBarUpdate && node->Windows.Size == 1 && (
        node_flags & ImGuiDockNodeFlags_AutoHideTabBar) && !node->
      IsHiddenTabBar())
...
static void ImGui::DockNodeUpdateVisibleFlag(ImGuiDockNode *node) {
  // Update visibility flag
  bool is_visible = (node->ParentNode == NULL)
                      ? node->IsDockSpace()
                      : node->IsCentralNode();
...
static void ImGui::DockNodeUpdate(ImGuiDockNode *node) {
  ...
  if (node->IsRootNode()) {
    ...
    if (ImGuiDockNode *first_node_with_windows = results.FirstNodeWithWindows) {
      ...
      for (int n = 1; n < first_node_with_windows->Windows.Size; n++)
        if (first_node_with_windows->Windows[n]->WindowClass.
  ...
  if (node->Windows.Size <= 1 && node->IsRootNode() && node->IsLeafNode() && !
      node->IsDockSpace() && !g.IO.ConfigDockingTabBarOnSingleWindows) {
    if (node->Windows.Size == 1) {
    ...
    node->HasCloseButton = node->HasWindowMenuButton =
  ...
  if (node->IsDockSpace()) {
    ...
    node->EnableCloseButton = false;
    node->HasCloseButton = (node->Windows.Size > 0) && (
                             node_flags & ImGuiDockNodeFlags_NoWindowMenuButton)
                           == 0;
    node->HasWindowMenuButton = (node->Windows.Size > 0) && (
                                  node_flags &
                                  ImGuiDockNodeFlags_NoWindowMenuButton) == 0;
    for (int window_n = 0; window_n < node->Windows.Size; window_n++) {
    ...
    if (node->IsRootNode() && node->IsVisible) {
      ImGuiWindow *ref_window = (node->Windows.Size > 0)
                                  ? node->Windows[0]
                                  : NULL;
  ...
  if (node->IsRootNode())
    if (g.NavWindow && g.NavWindow->RootWindowDockStop->DockNode && g.NavWindow
        ->RootWindowDockStop->ParentWindow == host_window)
  ...
  if (central_node_hole_register_hit_test_hole) {
    ...
    ImRect central_hole(central_node->Pos,
    ...
    if (central_node_hole && !central_hole.IsInverted()) {
      SetWindowHitTestHole(host_window, central_hole.Min,
  ...
  if (host_window && node->IsVisible)
    if (node->IsRootNode() && (g.MovingWindow == NULL || g.MovingWindow->
                               RootWindow != host_window))
...
static int IMGUI_CDECL TabItemComparerByDockOrder(
    const void *lhs, const void *rhs) {
...
static void ImGui::DockNodeUpdateTabBar(ImGuiDockNode *node,
  ...
  else if (g.NavWindow && g.NavWindow->RootWindowForTitleBarHighlight ==
           host_window->RootWindow && root_node->LastFocusedNodeID == node->ID)
    is_focused = true;
  ...
  // Tab bar are automatically destroyed when a node gets hidden
  if (tab_bar == NULL) {
    DockNodeAddTabBar(node);
    tab_bar = node->TabBar;
  }
  ...
  const bool has_window_menu_button =
      (node_flags & ImGuiDockNodeFlags_NoWindowMenuButton) == 0;
  const bool has_close_button = (node_flags & ImGuiDockNodeFlags_NoCloseButton)
                                == 0;
  ...
  ImU32 title_bar_col = GetColorU32(host_window->Collapsed
                                      ...
                                      ? ImGuiCol_TitleBgActive
                                      : ImGuiCol_TitleBg);
  host_window->DrawList->AddRectFilled(title_bar_rect.Min, title_bar_rect.Max,
                                       title_bar_col,
  ...
  if (tab_bar_is_recreated && TabBarFindTabByID(tab_bar, node->SelectedTabID) !=
    ...
    tab_bar->SelectedTabId = tab_bar->NextSelectedTabId = tab_bar->Tabs.back().
                             Window->ID;
  ...
  if (has_close_button && node->VisibleWindow) {
    ...
    if (CloseButton(host_window->GetID("#CLOSE"),
                    title_bar_rect.GetTR() + ImVec2(
                        -style.FramePadding.x * 2.0f - button_sz, 0.0f)))
  ...
  if (g.HoveredWindow == host_window && g.HoveredId == 0 && IsMouseHoveringRect(
...
static bool ImGui::DockNodeIsDropAllowed(ImGuiWindow *host_window,
  ...
  if (root_payload->DockNodeAsHost && root_payload->DockNodeAsHost->
                                                    IsSplitNode())
    return true;
...
static void ImGui::DockNodeCalcTabBarLayout(const ImGuiDockNode *node,
  ...
  if (node->HasCloseButton) {
    r.Max.x -= g.FontSize;
    // +1.0f; // In DockNodeUpdateTabBar() we currently display a disabled close button even if there is none.
  }
  if (node->HasWindowMenuButton && g.Style.WindowMenuButtonPosition ==
      ImGuiDir_Left) {
    r.Min.x += g.FontSize;
    // + g.Style.ItemInnerSpacing.x; // <-- Adding ItemInnerSpacing makes the title text moves slightly when in a docking tab bar. Instead we adjusted RenderArrowDockMenu()
  } else if (node->HasWindowMenuButton && g.Style.WindowMenuButtonPosition ==
             ImGuiDir_Right) {
...
bool ImGui::DockNodeCalcDropRectsAndTestMousePos(
  ...
  if (outer_docking) {
    ...
    off = ImVec2(
        ImFloor(
            parent.GetWidth() * 0.5f - GetFrameHeightWithSpacing() * 0.0f -
            hs_h), ImFloor(
            parent.GetHeight() * 0.5f - GetFrameHeightWithSpacing() * 0.0f -
            hs_h));
  } else {
  ...
  ) { out_r = ImRect(c.x - hs_w, c.y - hs_w, c.x + hs_w, c.y + hs_w); } else if
  (dir == ImGuiDir_Up
  ) {
...
static void ImGui::DockNodePreviewDockCalc(ImGuiWindow *host_window,
                                           ImGuiDockNode *host_node,
                                           ImGuiWindow *root_payload,
  ...
  data->FutureNode.HasCloseButton = (host_node
                                       ? host_node->HasCloseButton
                                       : host_window->HasCloseButton) || (
                                      root_payload->HasCloseButton);
  data->FutureNode.HasWindowMenuButton = host_node
                                           ? true
                                           : ((host_window->Flags &
                                               ImGuiWindowFlags_NoCollapse) == 0
                                           );
  data->FutureNode.Pos = host_node ? ref_node_for_rect->Pos : host_window->Pos;
  data->FutureNode.Size = host_node
                            ? ref_node_for_rect->Size
                            : host_window->Size;

  // Figure out here we are allowed to dock
  ImGuiDockNodeFlags host_node_flags = host_node
                                         ? host_node->GetMergedFlags()
                                         : 0;
  const bool src_is_visibly_splitted =
      root_payload->DockNodeAsHost && root_payload->DockNodeAsHost->
                                                    IsSplitNode() && (
  ...
  if (host_node && (host_node_flags & ImGuiDockNodeFlags_NoDockingInCentralNode)
      && host_node->IsCentralNode())
    data->IsCenterAvailable = false;

  data->IsSidesAvailable = true;
  if ((host_node && (host_node_flags & ImGuiDockNodeFlags_NoSplit)) || g.IO.
      ConfigDockingNoSplit)
    data->IsSidesAvailable = false;
  if (!is_outer_docking && host_node && host_node->ParentNode == NULL &&
      host_node->IsCentralNode())
    data->IsSidesAvailable = false;
  ...
  data->IsDropAllowed = (data->SplitDir != ImGuiDir_None) || (data->
                          IsCenterAvailable);
  if (!is_explicit_target && !data->IsSplitDirExplicit && !g.IO.
  ...
  if (data->SplitDir != ImGuiDir_None) {
    ...
    ImGuiAxis split_axis = (split_dir == ImGuiDir_Left || split_dir ==
                            ImGuiDir_Right)
                             ? ImGuiAxis_X
                             : ImGuiAxis_Y;
    ...
    data->SplitRatio = (split_dir == ImGuiDir_Right || split_dir ==
                        ImGuiDir_Down)
                         ? (1.0f - split_ratio)
                         : (split_ratio);
  }
...
static void ImGui::DockNodePreviewDockRender(ImGuiWindow *host_window,
  ...
  const ImU32 overlay_col_main = GetColorU32(ImGuiCol_DockingPreview,
                                             is_transparent_payload
                                               ? 0.60f
                                               : 0.40f);
  const ImU32 overlay_col_drop = GetColorU32(ImGuiCol_DockingPreview,
                                             is_transparent_payload
                                               ? 0.90f
                                               : 0.70f);
  const ImU32 overlay_col_drop_hovered = GetColorU32(
      ImGuiCol_DockingPreview, is_transparent_payload ? 1.20f : 1.00f);
  const ImU32 overlay_col_lines = GetColorU32(ImGuiCol_NavWindowingHighlight,
                                              is_transparent_payload
                                                ? 0.80f
                                                : 0.60f);
  ...
  const bool can_preview_tabs = (
    root_payload->DockNodeAsHost == NULL || root_payload->DockNodeAsHost->
    Windows.Size > 0);
  ...
  if (data->IsDropAllowed && can_preview_tabs && data->SplitDir == ImGuiDir_None
    ...
    if (host_node && host_node->TabBar) {
      if (!host_node->IsHiddenTabBar() && !host_node->IsNoTabBar())
        tab_pos.x += host_node->TabBar->OffsetMax + g.Style.ItemInnerSpacing.x;
      ...
      tab_pos.x += g.Style.ItemInnerSpacing.x +
          TabItemCalcSize(host_window->Name, host_window->HasCloseButton).x;
    ...
    if (root_payload->DockNodeAsHost)
      IM_ASSERT(
        root_payload->DockNodeAsHost->Windows.Size == root_payload->
        DockNodeAsHost->TabBar->Tabs.Size);
    const int payload_count = root_payload->DockNodeAsHost
                                ? root_payload->DockNodeAsHost->TabBar->Tabs.
                                                Size
                                : 1;
    for (int payload_n = 0; payload_n < payload_count; payload_n++) {
  ...
  for (int dir = ImGuiDir_None; dir < ImGuiDir_COUNT; dir++) {
    if (!data->DropRectsDraw[dir + 1].IsInverted()) {
      ...
      ImU32 overlay_col = (data->SplitDir == (ImGuiDir)dir && data->
                           IsSplitDirExplicit)
...
void ImGui::DockNodeTreeSplit(ImGuiContext *ctx, ImGuiDockNode *parent_node,
  ...
  ImGuiDockNode *child_inheritor = (split_inheritor_child_idx == 0)
                                     ? child_0
                                     : child_1;
  ...
  child_1->SizeRef[split_axis] = ImFloor(
      size_avail - child_0->SizeRef[split_axis]);
...
void ImGui::DockNodeTreeUpdatePosSize(ImGuiDockNode *node, ImVec2 pos,
  ...
  if (child_0->IsVisible && child_1->IsVisible) {
    ...
    if (child_0->WantLockSizeOnce) {
      ...
      child_0_size[axis] = ImMin(size_avail - size_min_each,
                                 child_0->SizeRef[axis]);
      child_1_size[axis] = (size_avail - child_0_size[axis]);
    } else if (child_0->IsCentralNode() && child_1->SizeRef[axis] != 0.0f) {
      child_1_size[axis] = ImMin(size_avail - size_min_each,
      ...
      child_0_size[axis] = ImMax(size_min_each,
                                 ImFloor(size_avail * split_ratio + 0.5F));
      child_1_size[axis] = (size_avail - child_0_size[axis]);
    }
...
void ImGui::DockNodeTreeUpdateSplitter(ImGuiDockNode *node) {
  ...
  if (child_0->IsVisible && child_1->IsVisible) {
    ...
    if ((child_0->GetMergedFlags() | child_1->GetMergedFlags()) &
      ...
      resize_limits[1] = node->ChildNodes[1]->Pos[axis] + node->ChildNodes[1]->
                         Size[axis] - min_size;
        ...
        if (touching_nodes[0].Size > 0 && touching_nodes[1].Size > 0) {
          ...
          for (int side_n = 0; side_n < 2; side_n++)
            for (int touching_node_n = 0;
                 ...
                 ++) {
              ImGuiDockNode *touching_node = touching_nodes[side_n][
                touching_node_n];
              //ImDrawList* draw_list = node->HostWindow ? GetOverlayDrawList(node->HostWindow) : GetOverlayDrawList((ImGuiViewportP*)GetMainViewport());
              ...
              while (touching_node->ParentNode != node) {
                if (touching_node->ParentNode->SplitAxis == axis) {
                  ...
                  ImGuiDockNode *node_to_preserve = touching_node->ParentNode->
                      ChildNodes[side_n];
...
ImGuiDockNode *ImGui::DockNodeTreeFindFallbackLeafNode(ImGuiDockNode *node) {
  if (node->IsLeafNode())
    return node;
  if (ImGuiDockNode *leaf_node = DockNodeTreeFindFallbackLeafNode(
      node->ChildNodes[0]))
    return leaf_node;
  if (ImGuiDockNode *leaf_node = DockNodeTreeFindFallbackLeafNode(
      node->ChildNodes[1]))
...
ImGuiDockNode *
ImGui::DockNodeTreeFindNodeByPos(ImGuiDockNode *node, ImVec2 pos) {
  if (!node->IsVisible)
    return NULL;
  ...
  if (node->IsLeafNode())
    return node;
  if (ImGuiDockNode *hovered_node = DockNodeTreeFindNodeByPos(
      node->ChildNodes[0], pos))
    return hovered_node;
  if (ImGuiDockNode *hovered_node = DockNodeTreeFindNodeByPos(
      node->ChildNodes[1], pos))
    return hovered_node;
  ...
  if (node->IsDockSpace() && node->IsRootNode()) {
    if (node->CentralNode && node->IsLeafNode())
...
void ImGui::DockSpace(ImGuiID id, const ImVec2 &size_arg,
                      ImGuiDockNodeFlags flags,
...
ImGuiID ImGui::DockSpaceOverViewport(ImGuiViewport *viewport,
...
void ImGui::DockBuilderRemoveNodeChildNodes(ImGuiID root_id) {
  ...
  ImGuiDockNode *root_node = root_id
                               ? DockContextFindNodeByID(ctx, root_id)
                               : NULL;
  ...
  for (int n = 0; n < dc->Nodes.Data.Size; n++)
    if (ImGuiDockNode *node = (ImGuiDockNode *)dc->Nodes.Data[n].val_p) {
      bool want_removal = (root_id == 0) || (
                            node->ID != root_id && DockNodeGetRootNode(node)->ID
                            == root_id);
      if (want_removal) {
        if (node->IsCentralNode())
  ...
  for (int settings_n = 0; settings_n < ctx->SettingsWindows.Size; settings_n++)
    if (ImGuiID window_settings_dock_id = ctx->SettingsWindows[settings_n].
        DockId)
...
void ImGui::DockBuilderRemoveNodeDockedWindows(ImGuiID root_id,
  ...
  for (int n = 0; n < g.Windows.Size; n++) {
    ...
    bool want_removal = (root_id == 0) || (
                        ...
                        || (window->DockNodeAsHost && window->DockNodeAsHost->ID
                            == root_id);
    if (want_removal) {
...
ImGuiID ImGui::DockBuilderSplitNode(ImGuiID id, ImGuiDir split_dir,
  ...
  ImGuiID id_at_dir = node->ChildNodes[
    (split_dir == ImGuiDir_Left || split_dir == ImGuiDir_Up) ? 0 : 1]->ID;
  ImGuiID id_other = node->ChildNodes[
    (split_dir == ImGuiDir_Left || split_dir == ImGuiDir_Up) ? 1 : 0]->ID;
  if (out_id_at_dir)
...
void ImGui::DockBuilderCopyWindowSettings(const char *src_name,
  ...
  if (ImGuiWindow *dst_window = FindWindowByName(dst_name)) {
    ...
    if (src_window->ViewportId != 0 && src_window->ViewportId !=
        IMGUI_VIEWPORT_DEFAULT_ID) {
...
void ImGui::DockBuilderCopyDockSpace(ImGuiID src_dockspace_id,
  ...
  for (int remap_window_n = 0; remap_window_n < in_window_remap_pairs->Size;
    ...
    else if (ImGuiWindowSettings *src_window_settings = FindWindowSettings(
        src_window_id))
...
void ImGui::BeginDocked(ImGuiWindow *window, bool *p_open) {
  ...
  const bool auto_dock_node = (g.IO.ConfigDockingTabBarOnSingleWindows) && !(
                                window->Flags & (
                                  ImGuiWindowFlags_ChildWindow |
                                  ImGuiWindowFlags_NoTitleBar |
                                  ImGuiWindowFlags_NoDocking));
  if (auto_dock_node) {
    ...
    want_undock |= (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasPos) &&
        (window->SetWindowPosAllowFlags & g.NextWindowData.PosCond) && g.
        NextWindowData.PosUndock;
    if (want_undock) {
      DockContextProcessUndockWindow(ctx, window);
      return;
    }
  ...
  if (node->HostWindow && window->BeginOrderWithinContext < node->HostWindow->
  ...
  SetNextWindowSize(node->Size);
  g.NextWindowData.PosUndock = false;
  // Cancel implicit undocking of SetNextWindowPos()
  window->DockIsActive = true;
  window->DockTabIsVisible = false;
  ...
  if ((node->WantCloseAll || node->WantCloseTabID == window->ID) && p_open !=
      NULL)
    *p_open = false;
...
void ImGui::BeginAsDockableDragDropSource(ImGuiWindow *window) {
  ...
  bool is_drag_docking = (g.IO.ConfigDockingWithShift) || ImRect(
                             0, 0, window->SizeFull.x, GetFrameHeight()).
...
void ImGui::BeginAsDockableDragDropTarget(ImGuiWindow *window) {
  ...
  if (!payload->IsDataType(IMGUI_PAYLOAD_TYPE_WINDOW) || !DockNodeIsDropAllowed(
          window, *(ImGuiWindow **)payload->Data)) {
    EndDragDropTarget();
    return;
  }
  ...
  if (AcceptDragDropPayload(
    ...
    const ImRect explicit_target_rect = (node && node->TabBar && !node->
                                         IsHiddenTabBar() && !node->IsNoTabBar()
                                        )
                                          ? node->TabBar->BarRect
                                          : ImRect(
                                              window->Pos,
    ...
    const bool is_explicit_target =
        g.IO.ConfigDockingWithShift || IsMouseHoveringRect(
    ...
    if (do_preview && (node != NULL || allow_null_target_node)) {
      ...
      if (node && (node->ParentNode || node->IsCentralNode()))
        if (ImGuiDockNode *root_node = DockNodeGetRootNode(node)) {
          DockNodePreviewDockCalc(window, root_node, payload_window,
                                  &split_outer, is_explicit_target, true);
          if (split_outer.IsSplitDirExplicit)
            split_data = &split_outer;
        }
      DockNodePreviewDockCalc(window, node, payload_window, &split_inner,
...
static void ImGui::DockSettingsHandler_ReadLine(ImGuiContext *ctx,
  ...
  if (strncmp(line, "DockNode", 8) == 0) {
    ...
    return;
  if (sscanf(line, "ID=0x%08X%n", &node.ID, &r) == 1) { line += r; } else
    return;
  if (sscanf(line, " Parent=0x%08X%n", &node.ParentID, &r) == 1) {
    line += r;
    if (node.ParentID == 0)
      return;
  }
  ...
  ImGuiDockContext *dc = ctx->DockContext;
  if (node.ParentID != 0)
    if (ImGuiDockNodeSettings *parent_settings = DockSettingsFindNodeSettings(
        ctx, node.ParentID))
...
static void DockSettingsHandler_DockNodeToSettings(
  ...
  node_settings.SplitAxis = node->IsSplitNode()
                              ? (char)node->SplitAxis
                              : ImGuiAxis_None;
  ...
  node_settings.SizeRef = ImVec2ih((short)node->SizeRef.x,
                                   (short)node->SizeRef.y);
  ...
  for (int node_n = 0; node_n < dc->SettingsNodes.Size; node_n++) {
    ...
    buf->appendf("%*s%s%*s", node_settings->Depth * 2, "",
                 (node_settings->Flags & ImGuiDockNodeFlags_DockSpace)
                   ? "DockSpace"
                   : "DockNode ", (max_depth - node_settings->Depth) * 2, "");
    // Text align nodes to facilitate looking at .ini file
    buf->appendf(" ID=0x%08X", node_settings->ID);
    if (node_settings->ParentID)
      buf->appendf(" Parent=0x%08X SizeRef=%d,%d", node_settings->ParentID,
...
static void RenderViewportThumbnail(ImDrawList *draw_list,
                                    ImGuiViewportP *viewport,
                                    const ImRect &bb) {
  ImGuiContext &g = *GImGui;
  ImGuiWindow *window = g.CurrentWindow;
  ...
  float alpha_mul = (viewport->Flags & ImGuiViewportFlags_Minimized)
                      ? 0.30f
                      : 1.00f;
  window->DrawList->AddRectFilled(bb.Min, bb.Max,
                                  ImGui::GetColorU32(
                                      ImGuiCol_Border, alpha_mul * 0.40f));
  for (int i = 0; i != g.Windows.Size; i++) {
    ...
    if (thumb_window->SkipItems && (
          thumb_window->Flags & ImGuiWindowFlags_ChildWindow))
      // FIXME-DOCK: Skip hidden docked windows. Identify those betters.
      continue;
    ...
    ImRect thumb_r_scaled = ImRect(ImFloor(off + thumb_r.Min * scale),
                                   ImFloor(off + thumb_r.Max * scale));
    ImRect title_r_scaled = ImRect(ImFloor(off + title_r.Min * scale),
                                   ImFloor(
                                       off + ImVec2(
                                           title_r.Max.x,
                                           title_r.Min.y) * scale) + ImVec2(
                                       0, 5)); // Exaggerate title bar height
    thumb_r_scaled.ClipWithFull(bb);
    title_r_scaled.ClipWithFull(bb);
    const bool window_is_focused = (
      g.NavWindow && thumb_window->RootWindowForTitleBarHighlight == g.NavWindow
      ->RootWindowForTitleBarHighlight);
    window->DrawList->AddRectFilled(thumb_r_scaled.Min, thumb_r_scaled.Max,
                                    ImGui::GetColorU32(
                                        ImGuiCol_WindowBg, alpha_mul));
    window->DrawList->AddRectFilled(title_r_scaled.Min, title_r_scaled.Max,
                                    ImGui::GetColorU32(
                                        window_is_focused
                                          ? ImGuiCol_TitleBgActive
                                          : ImGuiCol_TitleBg, alpha_mul));
...
void ImGui::ShowMetricsWindow(bool *p_open) {
  ...
  struct Funcs {
    static ImRect GetWindowRect(ImGuiWindow *window, int rect_type) {
      if (rect_type == WRT_OuterRect) { return window->Rect(); } else if (
        rect_type == WRT_OuterRectClipped
      ) { return window->OuterRectClipped; } else if (rect_type == WRT_InnerRect
      ) { return window->InnerRect; } else if (rect_type == WRT_InnerClipRect
      ) { return window->InnerClipRect; } else if (rect_type == WRT_WorkRect
      ) { return window->WorkRect; } else if (rect_type == WRT_Contents) {
        ImVec2 min = window->InnerRect.Min - window->Scroll + window->
                     WindowPadding;
    ...
    static void NodeDrawList(ImGuiWindow *window, ImGuiViewportP *viewport,
                             ImDrawList *draw_list, const char *label) {
      bool node_open = ImGui::TreeNode(draw_list,
                                       "%s: '%s' %d vtx, %d indices, %d cmds",
                                       label,
                                       draw_list->_OwnerName
                                         ? draw_list->_OwnerName
      ...
      for (const ImDrawCmd *pcmd = draw_list->CmdBuffer.begin();
        ...
        ImGui::Text(
            "ElemCount: %d, ElemCount/3: %d, VtxOffset: +%d, IdxOffset: +%d",
            pcmd->ElemCount, pcmd->ElemCount / 3, pcmd->VtxOffset,
            pcmd->IdxOffset);
        ...
        while (clipper.Step())
          for (int prim = clipper.DisplayStart, idx_i =
                       elem_offset + clipper.DisplayStart * 3;
    ...
    static void NodeWindow(ImGuiWindow *window, const char *label) {
      ...
      ImGui::BulletText("Scroll: (%.2f/%.2f,%.2f/%.2f)", window->Scroll.x,
                        window->ScrollMax.x, window->Scroll.y,
                        window->ScrollMax.y);
      ImGui::BulletText(
      ...
      ImGui::BulletText("ViewportMonitor: %d",
                        window->Viewport
                          ...
                          : -1);
      ImGui::BulletText(
          "DockId: 0x%04X, DockOrder: %d, %s: 0x%p, Act: %d, Vis: %d",
      ...
      if (window->ColumnsStorage.Size > 0 && ImGui::TreeNode(
      ...
      ImGui::BulletText("Storage: %d bytes",
                        window->StateStorage.Data.Size * (int)sizeof(
                          ImGuiStorage::Pair));
      ImGui::TreePop();
    }
    ...
    static void NodeViewport(ImGuiViewportP *viewport) {
      ...
      if (ImGui::TreeNode((void *)(intptr_t)viewport->ID,
        ...
        ImGui::BulletText(
            "Pos: (%.0f,%.0f), Size: (%.0f,%.0f), Monitor: %d, DpiScale: %.0f%%",
            viewport->Pos.x, viewport->Pos.y, viewport->Size.x,
            viewport->Size.y, viewport->PlatformMonitor,
        ...
        ImGui::BulletText("Flags: 0x%04X =%s%s%s%s%s%s", viewport->Flags,
                          ...
                          (flags & ImGuiViewportFlags_NoRendererClear)
                            ? " NoRendererClear"
                            : "", (flags & ImGuiViewportFlags_Minimized)
                                    ? " Minimized"
                                    : "");
        for (int layer_i = 0; layer_i < IM_ARRAYSIZE(
                                  viewport->DrawDataBuilder.Layers); layer_i++)
          for (int draw_list_i = 0;
               draw_list_i < viewport->DrawDataBuilder.Layers[layer_i].Size;
               draw_list_i++)
            Funcs::NodeDrawList(NULL, viewport,
                                viewport->DrawDataBuilder.Layers[layer_i][
                                  draw_list_i], "DrawList");
        ImGui::TreePop();
      }
    }
  };
  ...
  if (ImGui::TreeNode("Viewport", "Viewports (%d)", g.Viewports.Size)) {
    ...
    if (g.PlatformIO.Monitors.Size > 0 && ImGui::TreeNode(
            "Monitors", "Monitors (%d)", g.PlatformIO.Monitors.Size)) {
      ImGui::TextWrapped(
          "(When viewports are enabled, imgui needs uses monitor data to position popup/tooltips so they don't straddle monitors.)");
      for (int i = 0; i < g.PlatformIO.Monitors.Size; i++) {
        const ImGuiPlatformMonitor &mon = g.PlatformIO.Monitors[i];
        ImGui::BulletText(
            "Monitor #%d: DPI %.0f%%\n MainMin (%.0f,%.0f), MainMax (%.0f,%.0f), MainSize (%.0f,%.0f)\n WorkMin (%.0f,%.0f), WorkMax (%.0f,%.0f), WorkSize (%.0f,%.0f)",
            i, mon.DpiScale * 100.0f,
            mon.MainPos.x, mon.MainPos.y, mon.MainPos.x + mon.MainSize.x,
            mon.MainPos.y + mon.MainSize.y, mon.MainSize.x, mon.MainSize.y,
            mon.WorkPos.x, mon.WorkPos.y, mon.WorkPos.x + mon.WorkSize.x,
  ...
  if (ImGui::TreeNode("Tools")) {
    ...
    show_windows_rects |= ImGui::Combo("##show_windows_rect_type",
                                       &show_windows_rect_type, wrt_rects_names,
                                       WRT_Count);
  ...
  struct Funcs {
    static void NodeDockNode(ImGuiDockNode *node, const char *label) {
      ...
      if (node->Windows.Size > 0)
        ...
        open = ImGui::TreeNode((void *)(intptr_t)node->ID,
                               "%s 0x%04X%s: %s split (vis: '%s')", label,
                               node->ID, node->IsVisible ? "" : " (hidden)",
                               (node->SplitAxis == ImGuiAxis_X)
                                 ? "horizontal"
                                 : (node->SplitAxis == ImGuiAxis_Y)
      ...
      if (open) {
        IM_ASSERT(
            node->ChildNodes[0] == NULL || node->ChildNodes[0]->ParentNode ==
            node);
        IM_ASSERT(
            node->ChildNodes[1] == NULL || node->ChildNodes[1]->ParentNode ==
            node);
        ImGui::BulletText("Pos (%.0f,%.0f), Size (%.0f, %.0f) Ref (%.0f, %.0f)",
                          node->Pos.x, node->Pos.y, node->Size.x, node->Size.y,
                          node->SizeRef.x, node->SizeRef.y);
        ImGui::BulletText("VisibleWindow: 0x%08X %s",
                          node->VisibleWindow ? node->VisibleWindow->ID : 0,
                          node->VisibleWindow
                            ? node->VisibleWindow->Name
                            : "NULL");
        ...
        ImGui::BulletText("Misc:%s%s%s%s",
                          ...
                          (g.FrameCount - node->LastFrameAlive < 2)
                            ...
                            : "", (g.FrameCount - node->LastFrameActive < 2)
                                    ? " IsActive"
                                    : "");
        if (ImGui::TreeNode("flags", "LocalFlags: 0x%04X SharedFlags: 0x%04X",
                            node->LocalFlags, node->SharedFlags)) {
          ImGui::CheckboxFlags("LocalFlags: NoSplit",
                               (ImU32 *)&node->LocalFlags,
                               ImGuiDockNodeFlags_NoSplit);
          ImGui::CheckboxFlags("LocalFlags: NoResize",
                               (ImU32 *)&node->LocalFlags,
                               ImGuiDockNodeFlags_NoResize);
          ImGui::CheckboxFlags("LocalFlags: NoTabBar",
                               (ImU32 *)&node->LocalFlags,
                               ImGuiDockNodeFlags_NoTabBar);
          ImGui::CheckboxFlags("LocalFlags: HiddenTabBar",
                               (ImU32 *)&node->LocalFlags,
    ...
    static void NodeTabBar(ImGuiTabBar *tab_bar) {
      ...
      p += ImFormatString(p, buf_end - p, "TabBar (%d tabs)%s",
                          tab_bar->Tabs.Size,
                          (tab_bar->PrevFrameVisible < ImGui::GetFrameCount() -
                           2)
                            ? " *Inactive*"
                            : "");
      ...
      if (ImGui::TreeNode(tab_bar, "%s", buf)) {
        for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++) {
          ...
          ImGui::Text("%02d%c Tab 0x%08X '%s'", tab_n,
                      ...
                      tab->Window ? tab->Window->Name : "N/A");
          ImGui::PopID();
        }
        ImGui::TreePop();
      }
  ...
  if (ImGui::TreeNode("Settings")) {
    ...
    for (int n = 0; n < g.SettingsWindows.Size; n++)
      if (g.SettingsWindows[n].DockId != 0)
        ImGui::BulletText("Window '%s' -> DockId %08X",
                          g.SettingsWindows[n].Name,
    ...
    for (int n = 0; n < dc->Nodes.Data.Size; n++)
      if (ImGuiDockNode *node = (ImGuiDockNode *)dc->Nodes.Data[n].val_p) {
        ...
        overlay_draw_list->AddRectFilled(pos - ImVec2(1, 1),
                                         ...
                                         IM_COL32(200, 100, 100, 255));
        overlay_draw_list->AddText(NULL, 0.0f, pos,
