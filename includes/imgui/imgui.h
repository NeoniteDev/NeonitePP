...
// Multi-viewport support: interface for Platform/Renderer back-ends + viewports to render
...
// Helper to parse and apply text filters (e.g. "aaaaa[,bbbb][,ccccc]")
struct ImGuiViewport;
// Viewport (generally ~1 per window to output to at the OS level. Need per-platform support to use multiple viewports)
struct ImGuiWindowClass;
// Window class (rare/advanced uses: provide hints to the platform back-end via altered viewport flags and parent/child info)
...
// -> enum ImGuiCond_            // Enum: A condition for Set*()
typedef int ImGuiDataType;
// -> enum ImGuiDataType_        // Enum: A primary data type
typedef int ImGuiDir;
// -> enum ImGuiDir_             // Enum: A cardinal direction
typedef int ImGuiKey;
// -> enum ImGuiKey_             // Enum: A key identifier (ImGui-side enum)
typedef int ImGuiNavInput;
// -> enum ImGuiNavInput_        // Enum: An input identifier for navigation
typedef int ImGuiMouseCursor;
// -> enum ImGuiMouseCursor_     // Enum: A mouse cursor identifier
typedef int ImGuiStyleVar;
// -> enum ImGuiStyleVar_        // Enum: A variable identifier for styling
typedef int ImDrawCornerFlags;
// -> enum ImDrawCornerFlags_    // Flags: for ImDrawList::AddRect*() etc.
typedef int ImDrawListFlags;
// -> enum ImDrawListFlags_      // Flags: for ImDrawList
typedef int ImFontAtlasFlags;
// -> enum ImFontAtlasFlags_     // Flags: for ImFontAtlas
typedef int ImGuiBackendFlags;
// -> enum ImGuiBackendFlags_    // Flags: for io.BackendFlags
typedef int ImGuiColorEditFlags;
// -> enum ImGuiColorEditFlags_  // Flags: for ColorEdit*(), ColorPicker*()
typedef int ImGuiColumnsFlags;
// -> enum ImGuiColumnsFlags_    // Flags: for Columns(), BeginColumns()
...
// -> enum ImGuiDragDropFlags_   // Flags: for *DragDrop*()
...
// -> enum ImGuiInputTextFlags_  // Flags: for InputText*()
...
// -> enum ImGuiTreeNodeFlags_   // Flags: for TreeNode*(),CollapsingHeader()
typedef int ImGuiViewportFlags;
// -> enum ImGuiViewportFlags_   // Flags: for ImGuiViewport
typedef int ImGuiWindowFlags;
// -> enum ImGuiWindowFlags_     // Flags: for Begin*()
...
namespace ImGui {
	...
	IMGUI_API void ShowLoader(bool* p_open = NULL);
	IMGUI_API void ShowMessagemBox(char* message = NULL, bool* p_open = NULL);
	...
	IMGUI_API void SetNextWindowSizeConstraints(const ImVec2& size_min,
		...
		NULL, void* custom_callback_data
		= NULL);
	...
	IMGUI_API bool
		ListBoxHeader(const char* label, const ImVec2& size = ImVec2(0, 0));
	...
	IMGUI_API bool
		BeginPopupContextItem(const char* str_id = NULL, int mouse_button = 1);
	...
	IMGUI_API bool
		BeginPopupContextVoid(const char* str_id = NULL, int mouse_button = 1);
	...
	IMGUI_API void DockSpace(ImGuiID id, const ImVec2& size = ImVec2(0, 0),
	...
	IMGUI_API int
		GetKeyPressedAmount(int key_index, float repeat_delay, float rate);
	...
	IMGUI_API ImVec2
		GetMouseDragDelta(int button = 0, float lock_threshold = -1.0f);
	...
	IMGUI_API void
		RenderPlatformWindowsDefault(void* platform_arg = NULL,
...
struct ImGuiIO {
	...
	ImVec2 DisplaySize;
	// <unset>          // Main display size, in pixels. This is for the default viewport. Use BeginViewport() for other viewports.
	float DeltaTime;
	...
	ImFontAtlas* Fonts;
	// <auto>           // Load, rasterize and pack one or more fonts into a single texture.
	...
	// = false      // [BETA] Make every single floating window display within a docking node.
	...
	// = false;         // Set to make all floating imgui windows always create their own viewport. Otherwise, they are merged into the main host viewports when overlapping it.
	...
	// = true           // [BETA] Disable default OS window decoration flag for secondary viewports. When a viewport doesn't want window decorations, ImGuiViewportFlags_NoDecoration will be set on it. Enabling decoration can create subsequent issues at OS levels (e.g. minimum window size).
...
struct ImGuiWindowClass {
	...
	ImGuiViewportFlags ViewportFlagsOverrideMask;
	// Viewport flags to override when a window of this class owns a viewport. This allows you to enforce OS decoration or task bar icon, override the defaults on a per-window basis.
	ImGuiViewportFlags ViewportFlagsOverrideValue;
	// Viewport flags values to override when a window of this class owns a viewport.
	bool DockingAllowUnclassed;
	// true = can be docked/merged with an unclassed window

	ImGuiWindowClass() {
		ClassId = 0;
		ParentViewportId = 0;
		ViewportFlagsOverrideMask = ViewportFlagsOverrideValue = 0x00;
...
namespace ImGui {
	...
	static inline void ShowTestWindow() { return ShowLoader(); }
...
// OBSOLETED in 1.63 (from Aug 2018): made the names consistent
...
struct ImDrawCmd {
	...
	// Start offset in vertex buffer. Pre-1.71 or without ImGuiBackendFlags_RendererHasVtxOffset: always 0. With ImGuiBackendFlags_RendererHasVtxOffset: may be >0 to support meshes larger than 64K vertices with 16-bits indices.
...
enum ImDrawCornerFlags_ {
	...
	// 0x8
	ImDrawCornerFlags_Top = ImDrawCornerFlags_TopLeft |
	ImDrawCornerFlags_TopRight,
	// 0x3
	ImDrawCornerFlags_Bot = ImDrawCornerFlags_BotLeft |
	ImDrawCornerFlags_BotRight,
	// 0xC
	ImDrawCornerFlags_Left = ImDrawCornerFlags_TopLeft |
...
struct ImFontAtlas {
	...
	int CustomRectIds[1];
	// Identifiers of custom texture rectangle used by ImFontAtlas/ImDrawList
...
struct ImFont {
	...
	// 8     // in  // = (0,0)    // Offset font rendering by xx pixels
	...
	// 2     // in  // = '?'      // Replacement glyph if one isn't found. Only set via SetFallbackChar()
	float Scale;
	...
	IMGUI_API void AddGlyph(ImWchar c, float x0, float y0, float x1, float y1,
		...
		float advance_x);
	IMGUI_API void
		AddRemapChar(ImWchar dst, ImWchar src, bool overwrite_dst = true);
...
struct ImGuiPlatformIO {
	...
	// (Optional) Setup window transparency
	void (*Platform_UpdateWindow)(ImGuiViewport* vp);
	// (Optional) Called in UpdatePlatforms(). Optional hook to allow the platform back-end from doing general book-keeping every frame.
	void (*Platform_RenderWindow)(ImGuiViewport* vp, void* render_arg);
	// (Optional) Setup for render
	void (*Platform_SwapBuffers)(ImGuiViewport* vp, void* render_arg);
	// (Optional) Call Present/SwapBuffers (platform side)
...
enum ImGuiViewportFlags_ {
	...
	ImGuiViewportFlags_CanHostOtherWindows = 1 << 8
...
struct ImGuiViewport {
	...
	// void* to hold low-level, platform-native window handle (e.g. the HWND) when using an abstraction layer like GLFW or SDL (where PlatformHandle would be a SDL_Window*)
	...
	ImGuiViewport() {
		...
		DrawData = NULL;
		ParentViewportId = 0;
		RendererUserData = PlatformUserData =
			PlatformHandle = PlatformHandleRaw = NULL;
