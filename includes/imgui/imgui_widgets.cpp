...
void ImGui::TextEx(const char* text, const char* text_end,
	...
	const ImVec2 text_pos(window->DC.CursorPos.x,
		window->DC.CursorPos.y + window->DC.
	...
	if (text_end - text > 2000 && !wrap_enabled) {
		...
		ImVec2 pos = text_pos;
		if (!g.LogEnabled) {
			int lines_skippable = (int)(
				(window->ClipRect.Min.y - text_pos.y) / line_height);
...
void ImGui::TextV(const char* fmt, va_list args) {
	...
	ImGuiContext& g = *GImGui;
	const char* text_end = g.TempBuffer + ImFormatStringV(
		g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt,
		args);
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args) {
	...
	const char* value_text_end = value_text_begin + ImFormatStringV(
		g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer),
		fmt, args);
...
void ImGui::BulletTextV(const char* fmt, va_list args) {
	...
	const char* text_end = text_begin + ImFormatStringV(
		g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt,
		args);
	const ImVec2 label_size = CalcTextSize(text_begin, text_end, false);
	const float text_base_offset_y = ImMax(
		0.0f, window->DC.CurrLineTextBaseOffset);
	...
	const ImRect bb(window->DC.CursorPos,
		window->DC.CursorPos + ImVec2(
			g.FontSize + (label_size.x > 0.0f
				...
				: 0.0f),
			ImMax(line_height, label_size.y)));
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered,
	...
	const bool flatten_hovered_children =
		(flags & ImGuiButtonFlags_FlattenChildren) && g.HoveredRootWindow ==
		window->RootWindow;
	...
	if (g.DragDropActive && (flags & ImGuiButtonFlags_PressedOnDragDropHold) && !(
		...
		if (IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem)) {
			...
			if (CalcTypematicPressedRepeatAmount(g.HoveredIdTimer + 0.0001f,
				g.HoveredIdTimer + 0.0001f - g.IO.
				DeltaTime, 0.01f, 0.70f))
	...
	if (hovered) {
		if (!(flags & ImGuiButtonFlags_NoKeyModifiers) || (
			...
			if ((flags & ImGuiButtonFlags_PressedOnClickRelease) && g.IO.MouseClicked[
				0]) {
			...
			if (((flags & ImGuiButtonFlags_PressedOnClick) && g.IO.MouseClicked[0]) ||
				((flags & ImGuiButtonFlags_PressedOnDoubleClick) && g.IO.
			...
			if ((flags & ImGuiButtonFlags_PressedOnRelease) && g.IO.MouseReleased[0]
				...
				if (!((flags & ImGuiButtonFlags_Repeat) && g.IO.MouseDownDurationPrev[0]
			...
			if ((flags & ImGuiButtonFlags_Repeat) && g.ActiveId == id && g.IO.
				MouseDownDuration[0] > 0.0f && IsMouseClicked(0, true))
				pressed = true;
		}
	...
	if (g.NavActivateDownId == id) {
		...
		if (nav_activated_by_code || nav_activated_by_inputs || g.ActiveId == id) {
			...
			g.ActiveIdAllowNavDirFlags =
				(1 << ImGuiDir_Left) | (1 << ImGuiDir_Right) | (1 << ImGuiDir_Up) | (
					1 << ImGuiDir_Down);
		}
	}
	...
	if (g.ActiveId == id) {
		...
		if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
			...
			if (g.IO.MouseDown[0]) {
				...
				if (hovered && (flags & ImGuiButtonFlags_PressedOnClickRelease) && !g.
					DragDropActive) {
					bool is_double_click_release =
						(flags & ImGuiButtonFlags_PressedOnDoubleClick) && g.IO.
						MouseDownWasDoubleClick[0];
					bool is_repeating_already =
						(flags & ImGuiButtonFlags_Repeat) && g.IO.MouseDownDurationPrev[0]
						>= g.IO.KeyRepeatDelay; // Repeat mode trumps <on release>
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos,
	...
	ImVec2 off = dock_node
		? ImVec2(
			(float)(int)(-g.Style.ItemInnerSpacing.x * 0.5f) + 0.5f,
			0.0f)
		: ImVec2(0.0f, 0.0f);
	...
	ImVec2 center = bb.GetCenter();
	if (hovered || held)
		window->DrawList->AddCircleFilled(center + off + ImVec2(0, -0.5f),
	...
	if (IsItemActive() && IsMouseDragging(0)) {
		...
		if (dock_node != NULL && dock_node->VisibleWindow && !(
			dock_node->VisibleWindow->Flags & ImGuiWindowFlags_NoMove)) {
			ImGuiDockNode* root_node = DockNodeGetRootNode(dock_node);
			if (root_node->OnlyNodeWithWindows != dock_node || (
		...
		if (can_extract_dock_node) {
			...
			if (g.IO.MouseDragMaxDistanceAbs[0].x > threshold_x || g.IO.
				MouseDragMaxDistanceAbs[0].y > threshold_y)
				DockContextQueueUndockNode(&g, dock_node);
		}
		else {
			ImVec2 backup_active_click_offset =
				g.ActiveIdClickOffset + (pos - window->Pos);
...
ImGuiID ImGui::GetScrollbarID(ImGuiWindow* window, ImGuiAxis axis) {
	return window->
		GetIDNoKeepAlive(axis == ImGuiAxis_X ? "#SCROLLX" : "#SCROLLY");
}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis,
	...
	float alpha = 1.0f;
	if ((axis == ImGuiAxis_Y) && bb_frame_height < g.FontSize + g.Style.
		FramePadding.y * 2.0f)
		alpha = ImSaturate(
	...
	const float grab_h_pixels = ImClamp(
		scrollbar_size_v * (size_avail_v / win_size_v), style.GrabMinSize,
		scrollbar_size_v);
	...
	if (held && allow_interaction && grab_h_norm < 1.0f) {
		...
		if (g.ActiveIdIsJustActivated) {
			...
			seek_absolute = (clicked_v_norm < grab_v_norm || clicked_v_norm >
				grab_v_norm + grab_h_norm);
			if (seek_absolute)
		...
		*p_scroll_v = (float)(int)(0.5f + scroll_v_norm * scroll_max);
...
void ImGui::Scrollbar(ImGuiAxis axis) {
	...
	ImDrawCornerFlags rounding_corners = (other_scrollbar_size <= 0.0f)
...
bool ImGui::Checkbox(const char* label, bool* v) {
	...
	IMGUI_TEST_ENGINE_ITEM_INFO(id, label,
		window->DC.ItemFlags |
		ImGuiItemStatusFlags_Checkable | (*v ?
	...
	if (overlay_size.x > 0.0f)
		RenderTextClipped(
			ImVec2(ImClamp(fill_br.x + style.ItemSpacing.x, bb.Min.x,
				...
				bb.Min.y), bb.Max, overlay, NULL, &overlay_size,
			ImVec2(0.0f, 0.5f), &bb);
}
...
bool ImGui::SplitterBehavior(const ImRect& bb, ImGuiID id, ImGuiAxis axis,
	...
	window->DC.ItemFlags |= ImGuiItemFlags_NoNav |
	...
	bb_interact.Expand(axis == ImGuiAxis_Y
		? ImVec2(0.0f, hover_extend)
	...
	if (held || (g.HoveredId == id && g.HoveredIdPreviousFrame == id && g.
		HoveredIdTimer >= hover_visibility_delay))
		SetMouseCursor(axis == ImGuiAxis_Y
			? ImGuiMouseCursor_ResizeNS
			: ImGuiMouseCursor_ResizeEW);

	ImRect bb_render = bb;
	if (held) {
		ImVec2 mouse_delta_2d = g.IO.MousePos - g.ActiveIdClickOffset - bb_interact.
			Min;
		float mouse_delta = (axis == ImGuiAxis_Y)
			? mouse_delta_2d.y
			: mouse_delta_2d.x;
...
static float CalcMaxPopupHeightFromItemCount(int items_count) {
	...
	if (items_count <= 0)
		return FLT_MAX;
	return (g.FontSize + g.Style.ItemSpacing.y) * items_count - g.Style.
		ItemSpacing.y + (g.Style.WindowPadding.y * 2);
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value,
	...
	if (!(flags & ImGuiComboFlags_NoPreview))
		window->DrawList->AddRectFilled(frame_bb.Min,
			ImVec2(value_x2, frame_bb.Max.y), frame_col,
			style.FrameRounding,
	...
	if (!(flags & ImGuiComboFlags_NoArrowButton)) {
		...
		RenderArrow(window->DrawList,
			ImVec2(value_x2 + style.FramePadding.y,
				frame_bb.Min.y + style.FramePadding.y), text_col,
			ImGuiDir_Down);
	}
	...
	if (has_window_size_constraint) {
		g.NextWindowData.Flags |= ImGuiNextWindowDataFlags_HasSizeConstraint;
		g.NextWindowData.SizeConstraintRect.Min.x = ImMax(
...
static const char* PatchFormatStringFloatToInt(const char* fmt) {
	if (fmt[0] == '%' && fmt[1] == '.' && fmt[2] == '0' && fmt[3] == 'f' && fmt[4]
		== 0)
...
bool ImGui::DataTypeApplyOpFromText(const char* buf,
	...
	if (data_type == ImGuiDataType_S32) {
		...
		if (data_type == ImGuiDataType_S8)
			...
			*(ImS16*)data_ptr = (ImS16)
			ImClamp(v32, (int)IM_S16_MIN, (int)IM_S16_MAX);
		else if (data_type == ImGuiDataType_U16)
			*(ImU16*)data_ptr = (ImU16)
			ImClamp(v32, (int)IM_U16_MIN, (int)IM_U16_MAX);
		else
			IM_ASSERT(0);
	}
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed,
	...
	ImGuiContext& g = *GImGui;
	const ImGuiAxis axis = (flags & ImGuiDragFlags_Vertical)
		? ImGuiAxis_Y
		: ImGuiAxis_X;
	...
	if (g.ActiveIdSource == ImGuiInputSource_Mouse && IsMousePosValid() && g.IO.
		MouseDragMaxDistanceSqr[0] > 1.0f * 1.0f) {
	...
	if (is_power) {
		FLOATTYPE v_cur_norm_curved = ImPow(
			(FLOATTYPE)(v_cur - v_min) / (FLOATTYPE)(v_max - v_min),
			(FLOATTYPE)1.0f / power);
		g.DragCurrentAccum -= (float)(
			v_cur_norm_curved - v_old_ref_for_accum_remainder);
	}
	else {
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* v,
	...
	if (!temp_input_is_active) {
		...
		if (focus_requested || clicked || double_clicked || g.NavActivateId == id ||
			...
			if (focus_requested || (clicked && g.IO.KeyCtrl) || double_clicked || g.
				NavInputId == id) {
	...
	// Drag behavior
	const bool value_changed = DragBehavior(id, data_type, v, v_speed, v_min,
		v_max, format, power,
		ImGuiDragFlags_None);
	if (value_changed)
		MarkItemEdited(id);
	...
	const char* value_buf_end = value_buf + DataTypeFormatString(
		value_buf, IM_ARRAYSIZE(value_buf), data_type,
		v, format);
...
bool ImGui::DragScalarN(const char* label, ImGuiDataType data_type, void* v,
	...
	for (int i = 0; i < components; i++) {
		...
		value_changed |= DragScalar("", data_type, v, v_speed, v_min, v_max, format,
			power);
		PopID();
		PopItemWidth();
...
float ImGui::SliderCalcRatioFromValueT(ImGuiDataType data_type, TYPE v,
	...
	const bool is_power = (power != 1.0f) && (
		data_type == ImGuiDataType_Float || data_type ==
		ImGuiDataType_Double);
	const TYPE v_clamped = (v_min < v_max)
		? ImClamp(v, v_min, v_max)
		: ImClamp(v, v_max, v_min);
	if (is_power) {
		if (v_clamped < 0.0f) {
			const float f = 1.0f - (float)(
				(v_clamped - v_min) / (ImMin((TYPE)0, v_max) - v_min));
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id,
	...
	if (is_power && v_min * v_max < 0.0f) {
		...
		const FLOATTYPE linear_dist_min_to_0 = ImPow(
			v_min >= 0 ? (FLOATTYPE)v_min : -(FLOATTYPE)v_min,
			(FLOATTYPE)1.0f / power);
		const FLOATTYPE linear_dist_max_to_0 = ImPow(
			v_max >= 0 ? (FLOATTYPE)v_max : -(FLOATTYPE)v_max,
			(FLOATTYPE)1.0f / power);
	...
	if (g.ActiveId == id) {
		...
		if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
			...
			if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
				...
				const int decimal_precision = is_decimal
		...
		if (set_new_value) {
			...
			v_new = RoundScalarWithFormatT<
				TYPE, SIGNEDTYPE>(format, data_type, v_new);
...
bool ImGui::SliderBehavior(const ImRect& bb, ImGuiID id,
	...
	case ImGuiDataType_Float:
		IM_ASSERT(
			*(const float*)v_min >= -FLT_MAX / 2.0f && *(const float*)v_max <= FLT_MAX
			/ 2.0f);
		return SliderBehaviorT<float, float, float>(bb, id, data_type, (float*)v,
			*(const float*)v_min,
			*(const float*)v_max, format,
			power, flags, out_grab_bb);
	case ImGuiDataType_Double:
		IM_ASSERT(
			*(const double*)v_min >= -DBL_MAX / 2.0f && *(const double*)v_max <=
			DBL_MAX / 2.0f);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v,
	...
	if (!temp_input_is_active) {
		const bool focus_requested = FocusableItemRegister(window, id);
		const bool clicked = (hovered && g.IO.MouseClicked[0]);
		if (focus_requested || clicked || g.NavActivateId == id || g.NavInputId ==
			id) {
			SetActiveID(id, window);
			SetFocusID(id, window);
			FocusWindow(window);
	...
	// Slider behavior
	ImRect grab_bb;
	const bool value_changed = SliderBehavior(frame_bb, id, data_type, v, v_min,
		v_max, format, power,
...
bool ImGui::VSliderScalar(const char* label, const ImVec2& size,
	...
	const bool hovered = ItemHoverable(frame_bb, id);
	if ((hovered && g.IO.MouseClicked[0]) || g.NavActivateId == id || g.NavInputId
		== id) {
		SetActiveID(id, window);
		SetFocusID(id, window);
		FocusWindow(window);
...
bool ImGui::TempInputTextScalar(const ImRect& bb, ImGuiID id, const char* label,
	...
	flags |= ((data_type == ImGuiDataType_Float || data_type ==
		ImGuiDataType_Double)
	...
	if (value_changed) {
		value_changed = DataTypeApplyOpFromText(data_buf,
...
bool ImGui::InputScalarN(const char* label, ImGuiDataType data_type, void* v,
	...
	for (int i = 0; i < components; i++) {
		...
		if (i > 0)
			SameLine(0, g.Style.ItemInnerSpacing.x);
		value_changed |= InputScalar("", data_type, v, step, step_fast, format,
			flags);
		PopID();
		PopItemWidth();
		v = (void*)((char*)v + type_size);
...
bool ImGui::InputInt(const char* label, int* v, int step, int step_fast,
	...
	const char* format = (flags & ImGuiInputTextFlags_CharsHexadecimal)
		? "%08X"
		: "%d";
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size,
	...
	return InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(250, 0), flags,
		callback, user_data);
}
...
bool ImGui::InputTextWithHint(const char* label, const char* hint, char* buf,
	...
	IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline));
	// call InputTextMultiline()
	return InputTextEx(label, hint, buf, (int)buf_size, ImVec2(250, 0), flags,
		callback, user_data);
}
...
namespace ImStb {
	...
	static int is_word_boundary_from_right(STB_TEXTEDIT_STRING* obj, int idx) {
		return idx > 0
			? (is_separator(obj->TextW[idx - 1]) && !is_separator(
				obj->TextW[idx]))
			: 1;
	}
...
#define STB_TEXTEDIT_MOVEWORDRIGHT  STB_TEXTEDIT_MOVEWORDRIGHT_IMPL
	...
	static bool STB_TEXTEDIT_INSERTCHARS(
		STB_TEXTEDIT_STRING* obj, int pos, const ImWchar* new_text,
		int new_text_len) {
		...
		const int new_text_len_utf8 = ImTextCountUtf8BytesFromStr(
			new_text, new_text + new_text_len);
...
void ImGuiInputTextCallbackData::InsertChars(int pos, const char* new_text,
	...
	if (new_text_len + BufTextLen >= BufSize) {
		...
		int new_buf_size = BufTextLen + ImClamp(new_text_len * 4, 32,
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf,
	...
	IM_ASSERT(
		!((flags & ImGuiInputTextFlags_CallbackHistory) && (flags &
	...
	if (is_multiline) {
		...
		draw_window->DC.NavLayerActiveMaskNext |= draw_window->DC.
			NavLayerCurrentMask;
	...
	const bool focus_requested_by_code =
		focus_requested && (g.FocusRequestCurrWindow == window && g.
			FocusRequestCurrCounterAll == window->DC.
			FocusCounterAll);
	...
	const bool user_scroll_finish =
		is_multiline && state != NULL && g.ActiveId == 0 && g.
		ActiveIdPreviousFrame == GetScrollbarID(draw_window, ImGuiAxis_Y);
	const bool user_scroll_active =
		is_multiline && state != NULL && g.ActiveId == GetScrollbarID(
	...
	if (init_state && g.ActiveId != id) {
		...
		state->CurLenW = ImTextStrFromUtf8(state->TextW.Data, buf_size, buf, NULL,
			&buf_end);
	...
	const bool is_displaying_hint = (hint != NULL && (buf_display_from_state
		? state->TextA.Data
		: buf)[0] == 0);
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
		...
		const bool is_osx = io.ConfigMacOSXBehaviors;
		const bool is_shortcut_key = (is_osx
			? (io.KeySuper && !io.KeyCtrl)
			: (io.KeyCtrl && !io.KeySuper)) && !io.
			KeyAlt && !io.KeyShift;
		...
		const bool is_startend_key_down =
			is_osx && io.KeySuper && !io.KeyCtrl && !io.KeyAlt;
		...
		const bool is_paste = ((is_shortcut_key && IsKeyPressedMap(ImGuiKey_V)) || (
			is_shift_key_only && IsKeyPressedMap(
				ImGuiKey_Insert))) && !is_readonly;
			...
			if ((flags & (ImGuiInputTextFlags_CallbackCompletion |
				...
				if (event_flag) {
					...
					const int utf8_cursor_pos =
						callback_data.CursorPos = ImTextCountUtf8BytesFromStr(
							text, text + state->Stb.cursor);
					const int utf8_selection_start =
						callback_data.SelectionStart = ImTextCountUtf8BytesFromStr(
							text, text + state->Stb.select_start);
					const int utf8_selection_end =
						callback_data.SelectionEnd = ImTextCountUtf8BytesFromStr(
	...
	if (render_cursor || render_selection) {
...

		{
			...
			if (render_selection) {
				searches_input_ptr[1] = text_begin + ImMin(
		...
		if (render_cursor && state->CursorFollow) {
			...
			if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll)) {
				...
				if (cursor_offset.x < state->ScrollX)
					...
					state->ScrollX = (float)(int)(
						cursor_offset.x - size.x + scroll_increment_x);
			}
			else {
				state->ScrollX = 0.0f;
			}
		...
		if (render_selection) {
			...
			ImU32 bg_color =
				GetColorU32(ImGuiCol_TextSelectedBg, render_cursor ? 1.0f : 0.6f);
		...
		if (is_multiline)
			text_size = ImVec2(
				size.x, InputTextCalcTextLenAndLineCount(
					buf_display, &buf_display_end) * g.FontSize);
		// We don't need width
...
bool ImGui::ColorEdit4(const char* label, float col[4],
	...
	if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)
		) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0) {
		// RGB/HSV 0..255 Sliders
		const float w_item_one = ImMax(
			1.0f, (float)(int)((w_items_all - (style.ItemInnerSpacing.x) * (
				components - 1)) / (float)components));
		const float w_item_last = ImMax(
			1.0f, (float)(int)(w_items_all - (w_item_one + style.ItemInnerSpacing.x)
				* (components - 1)));
	...
	if (picker_active_window && g.ActiveId != 0 && g.ActiveIdWindow ==
...
void ImGui::RenderColorRectWithAlphaCheckerboard(
	...
	if (((col & IM_COL32_A_MASK) >> IM_COL32_A_SHIFT) < 0xFF) {
		...
		for (float y = p_min.y + grid_off.y; y < p_max.y; y += grid_step, yi++) {
			float y1 = ImClamp(y, p_min.y, p_max.y), y2 = ImMin(
				y + grid_step, p_max.y);
			if (y2 <= y1)
				continue;
			for (float x = p_min.x + grid_off.x + (yi & 1) * grid_step; x < p_max.x;
				x += grid_step * 2.0f) {
				float x1 = ImClamp(x, p_min.x, p_max.x), x2 = ImMin(
					x + grid_step, p_max.x);
				if (x2 <= x1)
					continue;
				...
				window->DrawList->AddRectFilled(ImVec2(x1, y1), ImVec2(x2, y2), col_bg2,
					...
					? rounding
					: 0.0f, rounding_corners_flags_cell);
			}
		}
	}
	else {
...
bool ImGui::ColorPicker4(const char* label, float col[4],
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel) {
		...
		if (IsItemActive()) {
			...
			if (ImTriangleContainsPoint(triangle_pa, triangle_pb, triangle_pc,
				...
				ImVec2 current_off_unrotated = ImRotate(current_off, cos_hue_angle,
					sin_hue_angle);
				if (!ImTriangleContainsPoint(triangle_pa, triangle_pb, triangle_pc,
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel) {
		...
		ImVec2 hue_cursor_pos(
			...
			wheel_center.y + sin_hue_angle * (wheel_r_inner + wheel_r_outer) *
			0.5f);
		...
		draw_list->AddRectFilledMultiColor(picker_pos,
			picker_pos + ImVec2(
				sv_picker_size, sv_picker_size),
			IM_COL32_WHITE, hue_color32, hue_color32,
			IM_COL32_WHITE);
		draw_list->AddRectFilledMultiColor(picker_pos,
			picker_pos +
			ImVec2(sv_picker_size, sv_picker_size),
			IM_COL32_BLACK_TRANS,
			IM_COL32_BLACK_TRANS, IM_COL32_BLACK,
			IM_COL32_BLACK);
		...
		RenderFrameBorder(ImVec2(bar0_pos_x, picker_pos.y),
			ImVec2(bar0_pos_x + bars_width,
				picker_pos.y + sv_picker_size), 0.0f);
	...
	if (alpha_bar) {
		...
		RenderColorRectWithAlphaCheckerboard(bar1_bb.Min, bar1_bb.Max,
			IM_COL32(0, 0, 0, 0),
			bar1_bb.GetWidth() / 2.0f,
			ImVec2(0.0f, 0.0f));
		draw_list->AddRectFilledMultiColor(bar1_bb.Min, bar1_bb.Max, col32_no_alpha,
			col32_no_alpha,
			col32_no_alpha & ~IM_COL32_A_MASK,
			col32_no_alpha & ~IM_COL32_A_MASK);
		float bar1_line_y = (float)(int)(
			picker_pos.y + (1.0f - alpha) * sv_picker_size + 0.5f);
	...
	if (value_changed && memcmp(backup_initial_col, col,
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col,
	...
	if (g.ActiveId == id && !(flags & ImGuiColorEditFlags_NoDragDrop) &&
		...
		if (flags & ImGuiColorEditFlags_NoAlpha)
			SetDragDropPayload(
				IMGUI_PAYLOAD_TYPE_COLOR_3F, &col_rgb, sizeof(float) * 3,
				ImGuiCond_Once);
		else
			SetDragDropPayload(
				IMGUI_PAYLOAD_TYPE_COLOR_4F, &col_rgb, sizeof(float) * 4,
				ImGuiCond_Once);
	...
	if (allow_opt_picker) {
		...
		for (int picker_type = 0; picker_type < 2; picker_type++) {
			...
			memcpy(&dummy_ref_col, ref_col,
				sizeof(float) * ((picker_flags & ImGuiColorEditFlags_NoAlpha)
					? 3
					: 4));
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags,
	...
	const float text_width = g.FontSize + (label_size.x > 0.0f
	...
	const ImRect interact_bb = display_frame
		...
		: ImRect(frame_bb.Min.x, frame_bb.Min.y,
			frame_bb.Min.x + text_width + style.
			ItemSpacing.x * 2, frame_bb.Max.y);
	...
	const ImU32 bg_col = GetColorU32((held && hovered)
		...
		: ImGuiCol_Header);
	const ImU32 text_col = GetColorU32(ImGuiCol_Text);
	const ImVec2 text_pos = frame_bb.Min + ImVec2(text_offset_x,
		text_base_offset_y);
	...
	if (display_frame) {
		...
		RenderArrow(window->DrawList,
			frame_bb.Min + ImVec2(padding.x, text_base_offset_y), text_col,
		...
		if (flags & ImGuiTreeNodeFlags_Bullet)
			RenderBullet(window->DrawList,
				frame_bb.Min + ImVec2(text_offset_x * 0.5f,
					g.FontSize * 0.50f +
					text_base_offset_y), text_col);
		else if (!is_leaf)
			RenderArrow(window->DrawList,
				frame_bb.Min + ImVec2(padding.x,
					g.FontSize * 0.15f +
					text_base_offset_y), text_col,
...
bool ImGui::CollapsingHeader(const char* label, bool* p_open,
	...
	if (p_open) {
		...
		float button_x = ImMax(window->DC.LastItemRect.Min.x,
			window->DC.LastItemRect.Max.x - g.Style.FramePadding.
			x * 2.0f - button_size);
...
bool ImGui::Selectable(const char* label, bool selected,
	...
	float w_draw = ImMax(label_size.x,
		window->Pos.x + max_x - window_padding.x - pos.x);
	ImVec2 size_draw(
		(size_arg.x != 0 && !(flags & ImGuiSelectableFlags_DrawFillAvailWidth))
		? size_arg.x
		: w_draw, size_arg.y != 0.0f ? size_arg.y : size.y);
	...
	if (!item_add) {
		if ((flags & ImGuiSelectableFlags_SpanAllColumns) && window->DC.
			CurrentColumns)
			PopColumnsBackground();
		return false;
	}
	...
	if (pressed || hovered)
		if (!g.NavDisableMouseHover && g.NavWindow == window && g.NavLayer == window
			->DC.NavLayerCurrent) {
			g.NavDisableHighlight = true;
...
bool ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg) {
	...
	ImVec2 size = CalcItemSize(size_arg, CalcItemWidth(),
		GetTextLineHeightWithSpacing() * 7.4f + style.
		ItemSpacing.y);
...
bool ImGui::ListBoxHeader(const char* label, int items_count,
	...
	ImVec2 size;
	size.x = 0.0f;
	size.y = GetTextLineHeightWithSpacing() * height_in_items_f + style.
		FramePadding.y * 2.0f;
...
// not using va_arg because they promote float to double
{
	...
	for (int i = 0; i < IM_ARRAYSIZE(Pos); i++)
		NextWidth += NextWidths[i] + ((i > 0 && NextWidths[i] > 0.0f)
			? Spacing
			: 0.0f);
...
bool ImGui::BeginMainMenuBar() {
	...
	SetNextWindowSize(ImVec2(viewport->Size.x,
		g.NextWindowData.MenuBarOffsetMinVal.y + g.
		FontBaseSize + g.Style.FramePadding.y));
	SetNextWindowViewport(viewport->ID);
	// Enforce viewport so we don't create our onw viewport when ImGuiConfigFlags_ViewportsNoMerge is set.
...
bool ImGui::BeginMenuBar() {
	...
	ImRect clip_rect(ImFloor(bar_rect.Min.x + window->WindowBorderSize + 0.5f),
		ImFloor(bar_rect.Min.y + window->WindowBorderSize + 0.5f),
		ImFloor(ImMax(bar_rect.Min.x,
			bar_rect.Max.x - ImMax(
...
void ImGui::EndMenuBar() {
	...
	if (NavMoveRequestButNoResultYet() && (
		...
		if (nav_earliest_child->ParentWindow == window && nav_earliest_child->DC.
			ParentLayoutType == ImGuiLayoutType_Horizontal && g.
	...
	PopClipRect();
	PopID();
	window->DC.MenuBarOffset.x = window->DC.CursorPos.x - window->MenuBarRect().
		Min.x;
...
bool ImGui::BeginMenu(const char* label, bool enabled) {
	...
	bool menuset_is_open = !(window->Flags & ImGuiWindowFlags_Popup) && (
		g.OpenPopupStack.Size > g.BeginPopupStack.Size && g.
		OpenPopupStack[g.BeginPopupStack.Size].OpenParentId
		== window->IDStack.back());
	...
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
		...
		pressed = Selectable(label, menu_is_open,
			...
			ImGuiSelectableFlags_DontClosePopups | (!enabled
		...
		pressed = Selectable(label, menu_is_open,
			...
			ImGuiSelectableFlags_DrawFillAvailWidth | (!enabled
				? ImGuiSelectableFlags_Disabled
				: 0), ImVec2(w, 0.0f));
		ImU32 text_col = GetColorU32(
			enabled ? ImGuiCol_Text : ImGuiCol_TextDisabled);
		RenderArrow(window->DrawList,
			pos + ImVec2(
				window->MenuColumns.Pos[2] + extra_w + g.FontSize * 0.30f,
				0.0f), text_col, ImGuiDir_Right);
	}
		...
		// (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
	{
		...
		ImGuiWindow* child_menu_window = (g.BeginPopupStack.Size < g.OpenPopupStack.
			Size&& g.OpenPopupStack[g.BeginPopupStack
			.Size].SourceWindow == window)
		...
		if (g.HoveredWindow == window && child_menu_window != NULL && !(
			...
			moving_toward_other_child_menu = ImTriangleContainsPoint(
				ta, tb, tc, g.IO.MousePos);
			//GetForegroundDrawList()->AddTriangleFilled(ta, tb, tc, moving_within_opened_triangle ? IM_COL32(0,128,0,128) : IM_COL32(128,0,0,128)); // [DEBUG]
		}
		if (menu_is_open && !hovered && g.HoveredWindow == window && g.
			HoveredIdPreviousFrame != 0 && g.HoveredIdPreviousFrame != id && !
		...
		else if (g.NavId == id && g.NavMoveRequest && g.NavMoveDir ==
			ImGuiDir_Down) // Nav-Down to open
		{
			want_open = true;
			NavMoveRequestCancel();
		}
	...
	IMGUI_TEST_ENGINE_ITEM_INFO(id, label,
		window->DC.ItemFlags |
		ImGuiItemStatusFlags_Openable | (menu_is_open ?
			ImGuiItemStatusFlags_Opened : 0));

	if (!menu_is_open && want_open && g.OpenPopupStack.Size > g.BeginPopupStack.
		Size) {
...
void ImGui::EndMenu() {
	...
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
	if (g.NavWindow && g.NavWindow->ParentWindow == window && g.NavMoveDir ==
		ImGuiDir_Left && NavMoveRequestButNoResultYet() && window->DC.LayoutType
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected,
	...
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
		...
		ImVec2 shortcut_size = shortcut
			? CalcTextSize(shortcut, NULL)
			: ImVec2(0.0f, 0.0f);
	...
	IMGUI_TEST_ENGINE_ITEM_INFO(window->DC.LastItemId, label,
		window->DC.ItemFlags |
		ImGuiItemStatusFlags_Checkable | (selected ?
			ImGuiItemStatusFlags_Checked : 0));
	return pressed;
}
...
ImGuiTabBar::ImGuiTabBar() {
	...
	ScrollingAnim = ScrollingTarget =
		ScrollingTargetDistToVisibility = ScrollingSpeed = 0.0f;
...
bool ImGui::BeginTabBar(const char* str_id, ImGuiTabBarFlags flags) {
	...
	ImRect tab_bar_bb = ImRect(window->DC.CursorPos.x, window->DC.CursorPos.y,
		...
		window->DC.CursorPos.y + g.FontSize + g.Style.
		FramePadding.y * 2);
	tab_bar->ID = id;
...
bool ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb,
	...
	// Layout
	ItemSize(ImVec2(0.0f /*tab_bar->OffsetMax*/, tab_bar->BarRect.GetHeight()));
	// Don't feed width back
	...
	if (dock_node != NULL) {
		...
		const float separator_min_x = tab_bar->BarRect.Min.x - ImFloor(
			window->WindowPadding.x * 0.5f);
		const float separator_max_x = tab_bar->BarRect.Max.x + ImFloor(
			window->WindowPadding.x * 0.5f);
		window->DrawList->AddLine(ImVec2(separator_min_x, y),
			ImVec2(separator_max_x, y), col, 1.0f);
	}
...
void ImGui::EndTabBar() {
	...
	if (tab_bar->VisibleTabWasSubmitted || tab_bar->VisibleTabId == 0 ||
		tab_bar_appearing)
		tab_bar->ContentsHeight = ImMax(
			window->DC.CursorPos.y - tab_bar->BarRect.Max.y, 0.0f);
	else
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar) {
	...
	if (tab_bar->ReorderRequestTabId != 0) {
		if (ImGuiTabItem* tab1 = TabBarFindTabByID(tab_bar,
	...
	for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++) {
		...
		if (most_recently_selected_tab == NULL || most_recently_selected_tab->
			LastFrameSelected < tab->LastFrameSelected)
		...
		const bool has_close_button = tab->Window
			? tab->Window->HasCloseButton
			: ((tab->Flags &
				ImGuiTabItemFlags_NoCloseButton) == 0);
	...
	const float width_avail = ImMax(
		tab_bar->BarRect.GetWidth() - initial_offset_x, 0.0f);
	float width_excess = (width_avail < width_total_contents)
		? (width_total_contents - width_avail)
		: 0.0f;
	if (width_excess > 0.0f && (tab_bar->Flags &
		ImGuiTabBarFlags_FittingPolicyResizeDown)) {
		// If we don't have enough room, resize down the largest tabs first
		ShrinkWidths(g.ShrinkWidthBuffer.Data, g.ShrinkWidthBuffer.Size,
			width_excess);
		for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
			tab_bar->Tabs[g.ShrinkWidthBuffer[tab_n].Index].Width = (float)(int)g.
			ShrinkWidthBuffer[tab_n].Width;
	}
	else {
	...
	const bool scrolling_buttons =
		(tab_bar->OffsetMax > tab_bar->BarRect.GetWidth() && tab_bar->Tabs.Size >
			1) && !(tab_bar->Flags & ImGuiTabBarFlags_NoTabListScrollingButtons) && (
				tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyScroll);
	if (scrolling_buttons)
		if (ImGuiTabItem* tab_to_select = TabBarScrollingButtons(tab_bar))
			// NB: Will alter BarRect.Max.x!
			scroll_track_selected_tab_id = tab_bar->SelectedTabId = tab_to_select->ID;
	...
	if (g.NavWindowingTarget != NULL && g.NavWindowingTarget->DockNode && g.
		NavWindowingTarget->DockNode->TabBar == tab_bar)
		tab_bar->VisibleTabId = scroll_track_selected_tab_id = g.NavWindowingTarget
		->ID;
...
void ImGui::TabBarAddTab(ImGuiTabBar* tab_bar, ImGuiTabItemFlags tab_flags,
	...
	IM_ASSERT(g.CurrentTabBar == NULL);
...
static void ImGui::TabBarScrollToTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab) {
	...
	float tab_x2 = tab->Offset + tab->Width + (order + 1 < tab_bar->Tabs.Size
		? margin
		: 1.0f);
	tab_bar->ScrollingTargetDistToVisibility = 0.0f;
	if (tab_bar->ScrollingTarget > tab_x1) {
		tab_bar->ScrollingTargetDistToVisibility = ImMax(
			tab_bar->ScrollingAnim - tab_x2, 0.0f);
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar) {
	...
	if (want_clip_rect)
		PushClipRect(tab_bar->BarRect.Min,
			tab_bar->BarRect.Max +
			ImVec2(g.Style.ItemInnerSpacing.x, 0.0f), true);
	...
	if (select_dir != 0)
		if (ImGuiTabItem* tab_item = TabBarFindTabByID(
			tab_bar, tab_bar->SelectedTabId)) {
			int selected_order = tab_bar->GetTabOrder(tab_item);
			int target_order = selected_order + select_dir;
			tab_to_select = &tab_bar->Tabs[
...
bool ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open,
	...
	const bool tab_bar_appearing = (tab_bar->PrevFrameVisible + 1 < g.FrameCount);
	const bool tab_bar_focused = (tab_bar->Flags & ImGuiTabBarFlags_IsFocused) !=
		0;
	...
	window->DC.CursorPos = tab_bar->BarRect.Min + ImVec2(
		(float)(int)tab->Offset - tab_bar->ScrollingAnim,
		0.0f);
	ImVec2 pos = window->DC.CursorPos;
	ImRect bb(pos, pos + size);
	...
	const bool single_floating_window_node =
		node && node->IsRootNode() && !node->IsDockSpace() && node->Windows.Size
		== 1 && g.IO.ConfigDockingTabBarOnSingleWindows;
	if (held && single_floating_window_node && IsMouseDragging(0, 0.0f)) {
		...
		if (docked_window != NULL && !(
			...
			bool undocking_tab = (g.DragDropActive && g.DragDropPayload.SourceId ==
				id);
			...
			if (!undocking_tab) //&& (!g.IO.ConfigDockingWithShift || g.IO.KeyShift)
			{
				...
				float threshold_y = (threshold_base * 1.5f) + ImClamp(
					(ImFabs(g.IO.MouseDragMaxDistanceAbs[0].x) -
						threshold_base * 2.0f) * 0.20f, 0.0f,
					threshold_base * 4.0f);
				...
				if (distance_from_edge_y >= threshold_y)
					...
					if ((tab_bar->ReorderRequestDir < 0 && tab_bar->GetTabOrder(tab) ==
						0) || (tab_bar->ReorderRequestDir > 0 && tab_bar->
							GetTabOrder(tab) == tab_bar->Tabs.Size - 1))
						undocking_tab = true;
			}
...
void ImGui::SetTabItemClosed(const char* label) {
	...
	bool is_within_manual_tab_bar =
		g.CurrentTabBar && !(g.CurrentTabBar->Flags &
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb,
	...
	const float rounding = ImMax(
		0.0f, ImMin(g.Style.TabRounding, width * 0.5f - 1.0f));
	const float y1 = bb.Min.y + 1.0f;
	const float y2 = bb.Max.y + ((flags & ImGuiTabItemFlags_Preview)
		? 0.0f
		: -1.0f);
...
bool ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list,
	...
	if (flags & ImGuiTabItemFlags_UnsavedDocument) {
		text_pixel_clip_bb.Max.x -= CalcTextSize(
			TAB_UNSAVED_MARKER, NULL, false).x;
	...
	if (close_button_id != 0)
		if (g.HoveredId == tab_id || g.HoveredId == close_button_id || g.
	...
	if (close_button_visible) {
		...
		if (CloseButton(close_button_id,
			ImVec2(
				bb.Max.x - frame_padding.x * 2.0f - close_button_sz,
				bb.Min.y)))
	...
	float ellipsis_max_x = close_button_visible
		? text_pixel_clip_bb.Max.x
		: bb.Max.x - 1.0f;
