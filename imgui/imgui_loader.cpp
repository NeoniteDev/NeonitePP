...
const char* Stages[] = {" Default", " Season 14", " Season 13", " Season 12", " Season 11", " Season 10 (X)", " Winterfest", " FortniteMares", " Summer", " BatMan", " Star Wars", " World Cup"};
...
void ImGui::ShowLoader(bool* p_open)
{
	...
	if (BeginTabBar("Neonite"), ImGuiTabBarFlags_AutoSelectNewTabs)
	{
		if (BeginTabItem("Main"))
		{
			...
			if (Button("Start Server"))
			{
				...
				{
					Server = CreateThread(nullptr, NULL, (LPTHREAD_START_ROUTINE)&server, nullptr, NULL, nullptr);
			...
			if (Button("Start Game"))
			{
				if (hLauncher == INVALID_HANDLE_VALUE)
				{
					...
					hLauncher = CreateThread(nullptr, NULL, (LPTHREAD_START_ROUTINE)&launcher, nullptr, NULL, nullptr);
			...
			if (Button("Close Game"))
			{
				if (hLauncher != INVALID_HANDLE_VALUE)
				{
					try
					{
						// this should kill your game at all :pepelaugh: -kemo
						// kek -sammy
	...
	if (BeginTabItem("Profile Settings"))
	{
		...
		if (bIsLockerOpen)
		{
			if (Begin("Locker", nullptr, ImGuiWindowFlags_NoDocking | ImGuiWindowFlags_NoCollapse))
			{
				...
				if (Button("+"))
				{
					...
					{
						MessageBoxA(nullptr, "The item added doesn't seem to be existing.", "Neonite++", ERROR);
					}
				...
				if (Button("-"))
				{
					if (!strstr(lowID.c_str(), "athena"))
					{
						MessageBoxA(nullptr, "This item isn't correct.", "Neonite++", ERROR);
					}
					else
					{
						...
						{
							MessageBoxA(nullptr, "This item doesn't seem to exist.", "Neonite++", ERROR);
						}
				...
				if (Button(("Erase")))
				{
					if (IDs.empty())
					{
						MessageBoxA(nullptr, "The list is already empty!", "Neonite++", ERROR);
					}
					else
					{
						IDs.clear();
...
/*
				...
				if (bHasImportedLocker)
				{
					...
					{
						char label[32];
						if (Selectable(IDs.at(i).c_str(), selected == i, ImGuiSelectableFlags_SpanAllColumns)) selected = i;
